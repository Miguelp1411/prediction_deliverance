...
def build_event_candidates(
    ...
    day_items = sorted(event_row['day_distribution'], key=lambda x: x[1], reverse=True)[:max(1, int(topk_days))]
    time_items = sorted(event_row['time_distribution'], key=lambda x: x[1], reverse=True)[:max(1, int(topk_times))]
