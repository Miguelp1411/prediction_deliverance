...
def _compute_anchor_start_bin(history: list[WeekRecord], task_id: int, occurrence_index: int) -> tuple[int, float, float]:
    same_occ = _task_occurrence_history_bins(history, task_id, occurrence_index)
    all_bins = _task_history_bins(history, task_id)
    if same_occ:
        anchor = int(round(float(np.median(same_occ))))
        return clip_start_bin(anchor), float(np.clip(len(same_occ) / max(len(history), 1), 0.0, 1.0)), float(len(same_occ))
    if all_bins:
        anchor = int(round(float(np.median(all_bins))))
        return clip_start_bin(anchor), float(np.clip(len(all_bins) / max(len(history) * MAX_OCCURRENCES_PER_TASK, 1), 0.0, 1.0)), 0.0
    return 0, 0.0, 0.0
...
def build_temporal_context(weeks: list[WeekRecord], target_week_index: int, task_id: int, occurrence_index: int, duration_min: float | None = None, duration_max: float | None = None) -> TemporalContext:
    ...
    anchor_start_bin, anchor_support, same_occ_support = _compute_anchor_start_bin(history, task_id, occurrence_index)
    anchor_day = int(anchor_start_bin // bins_per_day())
