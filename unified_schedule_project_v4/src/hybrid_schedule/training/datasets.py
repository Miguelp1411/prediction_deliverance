...
class UnifiedWeekSlotDataset(Dataset):
    def __init__(
        ...
        progress_label: str = 'Preparación datos - samples',
        ...
        progress = TerminalProgress(
            progress_label,
            len(self.indices),
            step_percent=progress_step_percent,
            enabled=show_progress,
        )
        for sample_idx, (database_id, robot_id, target_week_idx) in enumerate(self.indices, start=1):
            self.samples.append(self._build_sample(database_id, robot_id, target_week_idx))
            progress.update(sample_idx)
    ...
    def _build_sample(self, database_id: str, robot_id: str, target_week_idx: int) -> dict[str, Any]:
        ...
        raw_prototypes_by_task: dict[int, list[SlotPrototype]] = {}
        ...
        for task_idx in range(self.num_tasks):
            ...
            raw_prototypes_by_task[task_idx] = raw_protos
        ...
        slot_contexts_by_task: dict[int, dict[int, dict[str, Any]]] = {}
        for task_idx in range(self.num_tasks):
            task_cap = int(self.max_slots_by_task[task_idx])
            slot_contexts_by_task[task_idx] = build_task_slot_contexts(
                series=series,
                target_week_idx=target_week_idx,
                task_idx=task_idx,
                max_slots=task_cap,
                output_slots=self.max_slots,
                bin_minutes=self.bin_minutes,
                ordinal_prototypes=raw_prototypes_by_task[task_idx],
            )

        task_dispersion = {
            task_idx: float(slot_contexts_by_task[task_idx][0]['profile'].get('start_dispersion', 0.0))
            for task_idx in range(self.num_tasks)
        }
        ...
        for task_idx in range(self.num_tasks):
            ...
            for proto in prototypes_by_task[task_idx]:
                ...
                numeric = build_temporal_numeric_features(
                    ...
                    max_slots=task_cap,
                    bin_minutes=self.bin_minutes,
                    slot_context=slot_contexts_by_task[task_idx][slot_id],
