...
from hybrid_schedule.inference.predictor import UnifiedWeekPredictor
...
def run_holdout_backtest(
    ...
    device,
    output_dir: str | Path,
    feature_scaling: dict | None = None,
) -> tuple[list[dict], dict]:
    rows = []
    feature_scaling = dict(feature_scaling or {})
    for database_id, robot_id, target_week_idx in val_indices:
        ...
        predictor = UnifiedWeekPredictor(prefix_ctx, config, model, device, feature_scaling=feature_scaling)
        payload = predictor.predict_database_robot(database_id, robot_id)
        pred_events = [
            {
        ...
        metrics = evaluate_week(pred_events, true_events, bin_minutes=int(config['calendar']['bin_minutes']))
