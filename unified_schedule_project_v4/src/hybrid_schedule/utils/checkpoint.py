...
def load_checkpoint(path: str | Path, model, map_location: str | None = None, strict: bool = True) -> dict[str, Any]:
    payload = read_checkpoint(path, map_location=map_location)
    model.load_state_dict(payload['state_dict'], strict=bool(strict))
