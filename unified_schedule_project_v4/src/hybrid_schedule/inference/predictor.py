...
class UnifiedWeekPredictor:
    ...
    def predict_database_robot(self, database_id: str, robot_id: str) -> dict:
        ...
        if 'week_time_logits' in outputs:
            week_probs = torch.softmax(outputs['week_time_logits'][0], dim=-1).cpu().numpy()
        ...
        topk_week_bins = int(self.config['inference'].get('topk_week_bins', 20))
        ...
        for task_idx in range(len(self.context.task_names)):
            ...
            for i in chosen:
                start_distribution = []
                if week_probs is not None:
                    weekly_sorted = np.argsort(week_probs[i])[::-1][:max(1, topk_week_bins)]
                    start_distribution = [(int(j), float(week_probs[i, j])) for j in weekly_sorted]
                selected.append({
