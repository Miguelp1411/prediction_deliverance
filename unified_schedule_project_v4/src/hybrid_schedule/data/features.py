...
FEATURE_SCHEMA_VERSION = "v6_contextual_candidate_bank"
...
def build_global_context(df: pd.DataFrame, bin_minutes: int = 5, show_progress: bool = False) -> GlobalContext:
    ...
    series: dict[tuple[str, str], SeriesBundle] = {}
    grouped = list(df.groupby(['database_id', 'robot_id']))
    progress = TerminalProgress(
        'Preparacion datos - contexto por robot',
        len(grouped),
        enabled=show_progress,
    )
    for group_idx, ((database_id, robot_id), group) in enumerate(grouped, start=1):
...
def seasonal_count_baseline(
    ...
    lag_vals = lag_vals.astype(np.float32, copy=True)
    lag_mask = lag_mask.astype(np.float32, copy=True)
    if not use_lag52 and lag_vals.size >= 3:
        lag_vals[2] = 0.0
        lag_mask[2] = 0.0
    ...
    if not use_lag52 and weights.size >= 3:
        weights[2] = 0.0
...
def season_bucket_from_week_start(week_start: pd.Timestamp) -> str:
...
def week_type_bucket_from_week_start(week_start: pd.Timestamp) -> str:
    week_start = pd.Timestamp(week_start)
    week_end = week_start + pd.Timedelta(days=6)
    iso_week = int(week_start.isocalendar().week)
    if week_start.month != week_end.month:
        return 'transition_month'
    if iso_week in {1, 52, 53}:
        return 'year_edge'
    return 'regular'


def week_density_bucket(series: SeriesBundle, week_idx: int, quantiles: tuple[float, float] = (0.33, 0.66)) -> str:
    totals = np.asarray(series.counts.sum(axis=1), dtype=np.float32)
    if totals.size == 0:
        return 'mid'
    low, high = np.quantile(totals, quantiles)
    val = float(totals[int(np.clip(week_idx, 0, max(len(totals) - 1, 0)))])
    if val <= float(low):
        return 'low'
    if val >= float(high):
        return 'high'
...
def week_regime_signature(series: SeriesBundle, week_idx: int, lookback: int = 8) -> str:
    if len(series.week_starts) == 0:
        return 'unknown'
    week_idx = int(np.clip(week_idx, 0, len(series.week_starts) - 1))
    start = max(0, week_idx - max(1, int(lookback)) + 1)
    counts_window = series.counts[start:week_idx + 1].astype(np.float32)
    if counts_window.size == 0:
        return 'unknown'
    totals = counts_window.sum(axis=1)
    total_load = float(totals.mean()) if totals.size else 0.0
    active_tasks = float(((counts_window > 0).sum(axis=1)).mean()) if counts_window.size else 0.0

    recent_starts = series.mean_start[start:week_idx + 1].astype(np.float32)
    nonzero_starts = recent_starts[recent_starts > 0]
    mean_start = float(np.median(nonzero_starts)) if nonzero_starts.size else 0.0

    global_totals = np.asarray(series.counts.sum(axis=1), dtype=np.float32)
    global_active = np.asarray((series.counts > 0).sum(axis=1), dtype=np.float32)
    global_mean_start = np.asarray(series.mean_start, dtype=np.float32)
    global_nonzero_starts = global_mean_start[global_mean_start > 0]

    load_bucket = 'high' if total_load >= float(np.median(global_totals)) else 'low'
    active_bucket = 'wide' if active_tasks >= float(np.median(global_active)) else 'narrow'
    start_ref = float(np.median(global_nonzero_starts)) if global_nonzero_starts.size else 0.0
    start_bucket = 'late' if mean_start >= start_ref else 'early'
    return f'{load_bucket}|{active_bucket}|{start_bucket}'


def task_slot_prototypes_contextual(
    series: SeriesBundle,
    target_week_idx: int | None,
    task_idx: int,
    max_slots: int = 32,
    ...
    week_type_filter: str | None = None,
    density_filter: str | None = None,
    regime_filter: str | None = None,
    regime_lookback: int = 8,
) -> list[SlotPrototype]:
    if target_week_idx is None:
        target_week_idx = len(series.week_starts)
    start = max(0, target_week_idx - max_weeks)
    ...
    for week_idx in range(start, target_week_idx):
        week_start = pd.Timestamp(series.week_starts[week_idx])
        if season_filter is not None and season_bucket_from_week_start(week_start) != season_filter:
            continue
        if week_type_filter is not None and week_type_bucket_from_week_start(week_start) != week_type_filter:
            continue
        if density_filter is not None and week_density_bucket(series, week_idx) != density_filter:
            continue
        if regime_filter is not None and week_regime_signature(series, week_idx, lookback=regime_lookback) != regime_filter:
            continue
        task_events = sorted([e for e in series.events[week_idx] if e.task_idx == task_idx], key=lambda e: e.start_bin)
        if task_events:
            weeks_with_task += 1
        for rank, event in enumerate(task_events[:max_slots]):
            slot_starts.setdefault(rank, []).append(int(event.start_bin))
    ...
    if not slot_starts:
        return task_slot_prototypes(series, target_week_idx, task_idx, max_slots=max_slots, max_weeks=max_weeks)
...
def build_task_slot_contexts(
    ...
    bin_minutes: int = 5,
    ordinal_prototypes: list[SlotPrototype] | None = None,
) -> dict[int, dict[str, Any]]:
    """
    Calcula los contextos temporales de todos los slots de una tarea en una sola pasada.

    Es equivalente a llamar repetidamente a build_temporal_slot_context(..., slot_id=N),
    pero evita recalcular para cada slot el perfil de tarea, los prototipos históricos y
    las asignaciones evento-prototipo.
    """
    if target_week_idx is None:
        target_week_idx = len(series.week_starts)

    target_week_idx = int(target_week_idx)
    max_slots = int(max_slots)
    output_slots = int(output_slots if output_slots is not None else max_slots)

    bins_per_day = int(24 * 60 / bin_minutes)
    bins_per_week = int(7 * bins_per_day)
    slot_scale = float(max(1, max_slots - 1))
    max_start_norm = float(max(1, 7 * bins_per_day - 1))

    default_slot_stats = {
        'offset_mean_days': 0.0,
        'offset_mean_local': 0.0,
        'offset_std_local': 0.0,
        'slot_support': 0.0,
    }
    default_ordinal_stats = {
        'ordinal_support': 0.0,
        'ordinal_start_median_norm': 0.0,
        'ordinal_start_std_norm': 0.0,
        'ordinal_day_mode_norm': 0.0,
    ...
    # Valores comunes para todos los slots de una misma tarea.
    profile = task_temporal_profile(
        series,
        target_week_idx,
        task_idx,
        bin_minutes=bin_minutes,
    )
    duration_med = task_duration_median(
        series,
        target_week_idx,
        task_idx,
    )
    ...
    # ------------------------------------------------------------------
    # Estadísticas recientes equivalentes a slot_recent_start_offsets(...)
    # para todos los slots.
    # ------------------------------------------------------------------
    recent_max_weeks = 26
    recent_start = max(0, target_week_idx - recent_max_weeks)

    recent_prototypes = task_slot_prototypes(
        series,
        target_week_idx,
        task_idx,
        max_slots=max_slots,
        max_weeks=recent_max_weeks,
    )
    recent_proto_map = {int(p.slot_id): p for p in recent_prototypes}
    offsets_by_slot: dict[int, list[int]] = {slot_id: [] for slot_id in recent_proto_map}

    for week_events in series.events[recent_start:target_week_idx]:
        task_events = [e for e in week_events if int(e.task_idx) == int(task_idx)]
        if not task_events:
            continue

        assignments = assign_events_to_prototypes(task_events, recent_prototypes)
        for assigned_slot_id, event, _proto in assignments:
            assigned_slot_id = int(assigned_slot_id)
            anchor = recent_proto_map.get(assigned_slot_id)
            if anchor is None:
                continue
            offsets_by_slot.setdefault(assigned_slot_id, []).append(
                int(event.start_bin) - int(anchor.center_bin)
            )

    slot_stats_by_slot: dict[int, dict[str, float]] = {}
    for slot_id in range(output_slots):
        anchor = recent_proto_map.get(int(slot_id))
        if anchor is None:
            slot_stats_by_slot[slot_id] = dict(default_slot_stats)
            continue

        offsets = offsets_by_slot.get(int(slot_id), [])
        if not offsets:
            slot_stats_by_slot[slot_id] = {
                'offset_mean_days': 0.0,
                'offset_mean_local': 0.0,
                'offset_std_local': 0.0,
                'slot_support': float(anchor.support),
    ...
    # ------------------------------------------------------------------
    # Estadísticas ordinales equivalentes a task_slot_ordinal_stats(...)
    # para todos los slots.
    # ------------------------------------------------------------------
    ordinal_max_weeks = 104
    ordinal_start = max(0, target_week_idx - ordinal_max_weeks)

    if ordinal_prototypes is None:
        ordinal_prototypes = task_slot_prototypes(
            series,
            target_week_idx,
            task_idx,
            max_slots=max_slots,
            max_weeks=ordinal_max_weeks,
        )

    ordinal_proto_map = {int(p.slot_id): p for p in ordinal_prototypes}
    starts_by_slot: dict[int, list[int]] = {slot_id: [] for slot_id in ordinal_proto_map}
    day_hist_by_slot: dict[int, np.ndarray] = {
        slot_id: np.zeros(7, dtype=np.float32)
        for slot_id in ordinal_proto_map
    }

    active_weeks = 0
    total_weeks = max(1, target_week_idx - ordinal_start)

    if ordinal_prototypes:
        for week_events in series.events[ordinal_start:target_week_idx]:
            task_events = [e for e in week_events if int(e.task_idx) == int(task_idx)]
            if task_events:
                active_weeks += 1

            assignments = assign_events_to_prototypes(task_events, ordinal_prototypes)
            for assigned_slot_id, event, _proto in assignments:
                assigned_slot_id = int(assigned_slot_id)
                if assigned_slot_id not in ordinal_proto_map:
                    continue

                starts_by_slot.setdefault(assigned_slot_id, []).append(int(event.start_bin))
                day_hist_by_slot.setdefault(
                    assigned_slot_id,
                    np.zeros(7, dtype=np.float32),
                )[int(event.start_time.dayofweek)] += 1.0

    ordinal_stats_by_slot: dict[int, dict[str, float]] = {}
    for slot_id in range(output_slots):
        if int(slot_id) not in ordinal_proto_map:
            ordinal_stats_by_slot[slot_id] = dict(default_ordinal_stats)
            continue

        starts = starts_by_slot.get(int(slot_id), [])
        day_hist = day_hist_by_slot.get(int(slot_id), np.zeros(7, dtype=np.float32))
        day_sum = float(day_hist.sum())

        if day_sum > 0:
            day_probs = day_hist / day_sum
            day_entropy = _normalized_entropy(day_probs)
            day_mode = float(day_hist.argmax() / 6.0)
        else:
            day_entropy = 0.0
            day_mode = 0.0

        ordinal_stats_by_slot[slot_id] = {
            'ordinal_support': float(len(starts) / max(1, active_weeks)),
            'ordinal_start_median_norm': (
                float(np.median(starts) / max(1, bins_per_week - 1))
                if starts else 0.0
            ),
            'ordinal_start_std_norm': (
                float(np.std(np.asarray(starts, dtype=np.float32)) / max(1, bins_per_week - 1))
                if starts else 0.0
            ),
            'ordinal_day_mode_norm': day_mode,
            'ordinal_day_entropy': day_entropy,
            'ordinal_active_weeks_norm': float(active_weeks / total_weeks),
        }
    ...
    return {
        slot_id: {
            'profile': profile,
            'slot_stats': slot_stats_by_slot.get(slot_id, dict(default_slot_stats)),
            'ordinal_stats': ordinal_stats_by_slot.get(slot_id, dict(default_ordinal_stats)),
            'duration_med': float(duration_med),
            'bins_per_day': bins_per_day,
            'slot_scale': slot_scale,
            'max_start_norm': max_start_norm,
        }
        for slot_id in range(output_slots)
    }
...
def build_occurrence_numeric_features(
    series: SeriesBundle,
    target_week_idx: int | None,
    task_idx: int,
    template_count: int,
    ...
    seasonal = seasonal_count_baseline(series, target_week_idx, task_idx, template_count, use_lag52=use_lag52)
    ...
    info['use_lag52'] = 1.0 if use_lag52 else 0.0
    return numeric, info
...
def build_temporal_candidate_features(
    ...
    return np.asarray([
        ...
        float(plan_features.get('week_order_pct', 0.0)),
        float(plan_features.get('day_order_pct', 0.0)),
        float(candidate_meta.get('is_slot_source', 0.0)),
        float(candidate_meta.get('is_task_source', 0.0)),
        float(candidate_meta.get('is_seasonal_source', 0.0)),
        float(candidate_meta.get('is_week_type_source', 0.0)),
        float(candidate_meta.get('is_density_source', 0.0)),
        float(candidate_meta.get('is_regime_source', 0.0)),
        float(candidate_meta.get('is_precedence_source', 0.0)),
        float(candidate_meta.get('is_prototype_source', 0.0)),
        float(candidate_meta.get('season_match', 0.0)),
        float(candidate_meta.get('week_type_match', 0.0)),
        float(candidate_meta.get('density_match', 0.0)),
        float(candidate_meta.get('regime_match', 0.0)),
        float(candidate_meta.get('precedence_match', 0.0)),
        float(candidate_meta.get('neighbor_slot_match', 0.0)),
        float(candidate_meta.get('source_strength', 0.0)),
        float(candidate_meta.get('num_sources', 1.0)),
        float(candidate_meta.get('max_source_support', max(0.0, float(support_score)))),
        float(candidate_meta.get('context_consistency_score', 0.0)),
    ], dtype=np.float32)
