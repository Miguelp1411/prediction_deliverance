...
from hybrid_schedule.utils import get_device, macroblock_bins_from_minutes, num_macroblocks_per_day, save_checkpoint, seed_everything
...
def main() -> None:
    ...
    macroblock_bins = macroblock_bins_from_minutes(int(cfg['calendar']['bin_minutes']), int(cfg['models']['temporal'].get('macroblock_minutes', 60)))
    num_macroblocks = num_macroblocks_per_day(bins_per_day, macroblock_bins)
    ...
    temporal_ds_train = TemporalDataset(context, train_indices, window_weeks=int(cfg['calendar']['window_weeks']), topk_templates=int(cfg['calendar']['topk_templates']), bins_per_day=bins_per_day, macroblock_bins=macroblock_bins)
    temporal_ds_val = TemporalDataset(context, val_indices, window_weeks=int(cfg['calendar']['window_weeks']), topk_templates=int(cfg['calendar']['topk_templates']), bins_per_day=bins_per_day, macroblock_bins=macroblock_bins)
    ...
    tmp_model = TemporalResidualNet(
        ...
        fine_offset_bins=macroblock_bins,
    ).to(device)
    ...
    tmp_model, tmp_metrics, tmp_rows = fit_temporal_model(
        ...
        macroblock_bins=macroblock_bins,
