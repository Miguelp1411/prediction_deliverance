...
from auxiliary_corrector import AuxiliaryCorrector, train_and_save_auxiliary_corrector
from config import (
    CAP_INFERENCE_SCOPE,
    CHECKPOINT_DIR,
    DATA_PATH,
...
def summarize_auxiliary_corrector(
    ...
    for idx in week_indices:
        ...
        base_stats.append(evaluate_weekly_predictions(prepared.weeks[idx], base_pred))
        aux_stats.append(evaluate_weekly_predictions(prepared.weeks[idx], aux_pred))
    ...
    print(f"task_acc base -> aux   : {avg(base_stats, 'task_accuracy')*100:.1f}% -> {avg(aux_stats, 'task_accuracy')*100:.1f}%")
    print(f"hora exacta base -> aux: {avg(base_stats, 'time_exact_accuracy')*100:.1f}% -> {avg(aux_stats, 'time_exact_accuracy')*100:.1f}%")
    print(f"hora ±5m base -> aux   : {avg(base_stats, 'time_close_accuracy_5m')*100:.1f}% -> {avg(aux_stats, 'time_close_accuracy_5m')*100:.1f}%")
    print(f"MAE inicio base -> aux : {avg(base_stats, 'start_mae_minutes'):.2f} -> {avg(aux_stats, 'start_mae_minutes'):.2f} min")
    print(f"dur ±2m base -> aux    : {avg(base_stats, 'duration_close_accuracy')*100:.1f}% -> {avg(aux_stats, 'duration_close_accuracy')*100:.1f}%")
