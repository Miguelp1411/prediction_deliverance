...
from proyectos_anteriores.project_03.config import num_global_day_offset_classes, num_local_start_offset_classes
from proyectos_anteriores.project_03.models.blocks import SequenceContextEncoder
...
class TemporalAssignmentModel(nn.Module):
    def __init__(
        ...
        self.day_embedding = nn.Embedding(7, day_embed_dim)
    ...
    def forward_with_context(
        ...
        day_vec = self.day_embedding(anchor_day)
        hist_input = torch.cat([history_features, predicted_count_norm.unsqueeze(-1)], dim=-1)
        hist_vec = self.history_mlp(hist_input)
        fused = self.fusion(torch.cat([sequence_context, task_vec, occ_vec, day_vec, hist_vec], dim=-1))
        return {
