...
from proyectos_anteriores.project_03.config import (
    ...
    TMP_WEIGHT_DECAY,
    TRAIN_RATIO,
    num_global_day_offset_classes,
    num_local_start_offset_classes,
    num_time_bins,
)
from proyectos_anteriores.project_03.data.datasets import OccurrenceDataset, TemporalDataset, build_split_indices
from proyectos_anteriores.project_03.data.io import load_tasks_dataframe
from proyectos_anteriores.project_03.data.preprocessing import prepare_data, serialize_metadata
from proyectos_anteriores.project_03.evaluation.weekly_stats import evaluate_weekly_predictions
from proyectos_anteriores.project_03.models.occurrence_model import TaskOccurrenceModel
from proyectos_anteriores.project_03.models.temporal_model import TemporalAssignmentModel
from proyectos_anteriores.project_03.predict import predict_next_week
from proyectos_anteriores.project_03.training.engine import evaluate_epoch, fit_model
from proyectos_anteriores.project_03.training.losses import OccurrenceLoss, TemporalLoss
from proyectos_anteriores.project_03.training.metrics import occurrence_metrics, temporal_metrics
from proyectos_anteriores.project_03.utils.serialization import save_checkpoint
...
def week_to_task_list(week_record):
    ...
    for events in week_record.events_by_task.values():
        for e in events:
            items.append({'task_name': e.task_name, 'start_bin': e.start_bin, 'duration': e.duration_minutes})
...
def print_week_example(prepared, week_idx, pred_week, stats, max_items: int = 8):
    ...
    print(
        '    '
        f"tareas={stats['total_tasks']} | task_acc={stats['task_accuracy']*100:.1f}% | "
        f"hora_exacta={stats['time_exact_accuracy']*100:.1f}% | hora_±10m={stats['time_close_accuracy']*100:.1f}% | "
...
def main():
    ...
    print(f'  clases offset día         : {num_global_day_offset_classes()}')
    print(f'  clases offset local       : {num_local_start_offset_classes()}')
    ...
    occ_state = fit_model(occurrence_model, occ_train_loader, occ_val_loader, occ_optimizer, occ_scheduler, occ_loss_fn, occurrence_metrics, device, OCC_MAX_EPOCHS, OCC_PATIENCE, 'OccurrenceModel')
    ...
    tmp_state = fit_model(temporal_model, tmp_train_loader, tmp_val_loader, temporal_optimizer, temporal_scheduler, temporal_loss_fn, temporal_metrics_wrapper, device, TMP_MAX_EPOCHS, TMP_PATIENCE, 'TemporalModel')
    ...
    summarize_weekly_predictions('train', split.train_target_week_indices[-8:], prepared, occurrence_model, temporal_model, device, 8)
    summarize_weekly_predictions('validación', split.val_target_week_indices[:8], prepared, occurrence_model, temporal_model, device, 8)
