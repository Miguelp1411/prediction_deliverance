...
from proyectos_anteriores.project_03.config import TRAIN_RATIO, WINDOW_WEEKS
from proyectos_anteriores.project_03.data.preprocessing import (
    PreparedData,
...
class TemporalDataset(Dataset):
    def __init__(self, prepared: PreparedData, target_week_indices: list[int]):
        ...
        for week_idx in target_week_indices:
            ...
            target_day_offsets = build_target_day_offsets(prepared.weeks, week_idx)
            for task_id, events in target_week.events_by_task.items():
                ...
                for occurrence_index, event in enumerate(sorted_events):
                    ...
                    day_offset = int(target_day_offsets.get((task_id, occurrence_index), 0))
                    local_offset = int(event.start_bin) - int(context.anchor_start_bin) - day_offset
                    self.samples.append({
                        'sequence': seq,
                        'task_id': int(task_id),
                        ...
                        'target_day_offset_idx': int(global_day_offset_to_index(day_offset)),
                        'target_local_offset_idx': int(local_start_offset_to_index(local_offset)),
    ...
    def __getitem__(self, index: int):
        ...
        return {
            ...
            'target_day_offset_idx': torch.tensor(item['target_day_offset_idx'], dtype=torch.long),
            'target_local_offset_idx': torch.tensor(item['target_local_offset_idx'], dtype=torch.long),
