...
def prepare_data(
    ...
    inferred_max_occurrences_per_task = int(train_weeks_df.groupby(['week_start', 'task_id']).size().max()) if not train_weeks_df.empty else 1
    inferred_max_tasks_per_week = int(train_weeks_df.groupby('week_start').size().max()) if not train_weeks_df.empty else 1
    max_occurrences_per_task = _sanitize_cap(max_occurrences_per_task, inferred_max_occurrences_per_task)
    max_tasks_per_week = _sanitize_cap(max_tasks_per_week, inferred_max_tasks_per_week)
    ...
    for week_index, week_start in enumerate(week_starts):
        ...
        weeks.append(_compute_week_features(week_df, week_index, week_start, len(task_names), max_count_cap, max_tasks_per_week))
