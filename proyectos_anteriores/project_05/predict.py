...
from proyectos_anteriores.project_05.config import (
    BIN_MINUTES,
    CHECKPOINT_DIR,
    DATA_PATH,
...
from proyectos_anteriores.project_05.data.io import load_tasks_dataframe
from proyectos_anteriores.project_05.data.preprocessing import (
    build_temporal_context,
    denormalize_duration,
    prepare_data,
    week_to_feature_vector,
)
from proyectos_anteriores.project_05.models.occurrence_model import TaskOccurrenceModel
from proyectos_anteriores.project_05.models.temporal_model import TemporalAssignmentModel
from proyectos_anteriores.project_05.utils.serialization import load_checkpoint
...
def _ensure_checkpoint_compatibility(ckpt: dict, prepared, checkpoint_name: str) -> None:
    ...
    checkpoint_max_occurrences = metadata.get('max_occurrences_per_task', metadata.get('max_count_cap'))
    ...
    checkpoint_max_tasks = metadata.get('max_tasks_per_week')
...
def _extract_preprocessing_caps(occ_ckpt: dict, tmp_ckpt: dict) -> tuple[int, int]:
    occ_metadata = occ_ckpt.get('metadata', {}) or {}
    tmp_metadata = tmp_ckpt.get('metadata', {}) or {}

    occ_cap = _positive_int(
        tmp_metadata.get('max_occurrences_per_task', occ_metadata.get('max_occurrences_per_task', tmp_metadata.get('max_count_cap', occ_metadata.get('max_count_cap', DEFAULT_MAX_OCCURRENCES_PER_TASK)))),
        DEFAULT_MAX_OCCURRENCES_PER_TASK,
    )
    tasks_cap = _positive_int(
        tmp_metadata.get('max_tasks_per_week', occ_metadata.get('max_tasks_per_week', DEFAULT_MAX_TASKS_PER_WEEK)),
        DEFAULT_MAX_TASKS_PER_WEEK,
    )
...
def main():
    ...
    max_occurrences_per_task, max_tasks_per_week = _extract_preprocessing_caps(occ_ckpt, tmp_ckpt)
