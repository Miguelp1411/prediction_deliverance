...
def build_context_sequence_features(weeks: list[WeekRecord], target_week_index: int, window_weeks: int, num_tasks: int) -> np.ndarray:
    ...
    return np.concatenate([seq, repeated_lags], axis=1).astype(np.float32)
...
def build_target_occurrence_slot_assignments(
    ...
    return _assign_events_to_prototype_slots(target_week.events_by_task.get(task_id, []), prototypes, max_occurrences_per_task)
...
def build_prediction_occurrence_slots(
    ...
    ranked_slots = [
        int(proto.slot_id)
        for proto in sorted(prototypes, key=lambda proto: (-float(proto.support), int(proto.center_bin), int(proto.slot_id)))
    ]
    selected = ranked_slots[:predicted_count]
    next_slot = len(prototypes)
...
def _compute_anchor_start_bin(
    ...
    prototypes = _build_occurrence_prototypes(anchor_history, task_id, max_occurrences_per_task)
    same_occ_recent = _task_occurrence_history_bins(recent_history, task_id, occurrence_slot, prototypes, max_occurrences_per_task)
    same_occ_long = _task_occurrence_history_bins(anchor_history, task_id, occurrence_slot, prototypes, max_occurrences_per_task)
    ...
    if nearest_proto is not None:
        proto_bins = _task_occurrence_history_bins(anchor_history, task_id, int(nearest_proto.slot_id), prototypes, max_occurrences_per_task)
        if proto_bins:
...
def build_temporal_context(
    ...
    anchor_start_bin, anchor_support, same_occ_support, anchor_long_support, anchor_proto_match, prototypes = _compute_anchor_start_bin(
        history,
        anchor_history,
        task_id,
        occurrence_slot,
    ...
    recent_offsets = _collect_recent_offsets(history, task_id, occurrence_slot, anchor_start_bin, prototypes, max_occurrences_per_task)
    recent_same_day_offsets = _collect_recent_same_day_offsets(history, task_id, occurrence_slot, anchor_start_bin, prototypes, max_occurrences_per_task)
    ...
    lag26_start = _same_occurrence_start_bin_at_lag(weeks, target_week_index, task_id, occurrence_slot, 26, prototypes, max_occurrences_per_task)
    lag52_start = _same_occurrence_start_bin_at_lag(weeks, target_week_index, task_id, occurrence_slot, 52, prototypes, max_occurrences_per_task)
    hist_median_start = _same_occurrence_historical_median_bin(anchor_history, task_id, occurrence_slot, prototypes, max_occurrences_per_task)
