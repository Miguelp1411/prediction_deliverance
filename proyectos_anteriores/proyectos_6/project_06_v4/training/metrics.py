from __future__ import annotations

import torch

from proyectos_anteriores.proyectos_6.project_06_v4.config import (
    BIN_MINUTES,
    START_TOPK_TOLERANCE_BINS,
    bins_per_day,
)
from proyectos_anteriores.proyectos_6.project_06_v4.config import GLOBAL_DAY_OFFSET_RADIUS_BINS, LOCAL_START_OFFSET_RADIUS_BINS, num_time_bins


@torch.no_grad()
def occurrence_metrics(logits: torch.Tensor, target_counts: torch.Tensor) -> dict[str, float]:
    pred_counts = logits.argmax(dim=-1)
    exact_acc = (pred_counts == target_counts).float().mean().item() * 100.0
    count_mae = torch.abs(pred_counts.float() - target_counts.float()).mean().item()
    weekly_total_mae = torch.abs(pred_counts.sum(dim=-1).float() - target_counts.sum(dim=-1).float()).mean().item()

    pred_presence = pred_counts > 0
    true_presence = target_counts > 0
    tp = (pred_presence & true_presence).sum().item()
    fp = (pred_presence & ~true_presence).sum().item()
    fn = (~pred_presence & true_presence).sum().item()
    precision = tp / max(tp + fp, 1)
    recall = tp / max(tp + fn, 1)
    f1 = 2 * precision * recall / max(precision + recall, 1e-8)
    return {
        'count_exact_acc': exact_acc,
        'count_mae': count_mae,
        'weekly_total_mae': weekly_total_mae,
        'presence_precision': precision * 100.0,
        'presence_recall': recall * 100.0,
        'presence_f1': f1 * 100.0,
    }


def _decode_offsets(outputs: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
    pred_day_offset = outputs['day_offset_logits'].argmax(dim=-1).long() - GLOBAL_DAY_OFFSET_RADIUS_BINS
    pred_local_offset = outputs['local_offset_logits'].argmax(dim=-1).long() - LOCAL_START_OFFSET_RADIUS_BINS
    return pred_day_offset, pred_local_offset


@torch.no_grad()
def decode_predicted_start_bins(outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> torch.Tensor:
    pred_day_offset, pred_local_offset = _decode_offsets(outputs)
    anchor_start = batch['anchor_start_bin'].long()
    anchor_day = torch.div(anchor_start, bins_per_day(), rounding_mode='floor')
    anchor_local = torch.remainder(anchor_start, bins_per_day())
    pred_day = (anchor_day + pred_day_offset).clamp(min=0, max=6)
    pred_local = (anchor_local + pred_local_offset).clamp(min=0, max=bins_per_day() - 1)
    pred_start = pred_day * bins_per_day() + pred_local
    return pred_start.clamp(min=0, max=num_time_bins() - 1)


@torch.no_grad()
def temporal_metrics(outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], duration_span_minutes: float) -> dict[str, float]:
    pred_start = decode_predicted_start_bins(outputs, batch)
    target_start = batch['target_start_bin']
    diff = torch.abs(pred_start - target_start)
    pred_day_offset, pred_local_offset = _decode_offsets(outputs)
    true_day_offset = batch['target_day_offset_idx'].long() - GLOBAL_DAY_OFFSET_RADIUS_BINS
    true_local_offset = batch['target_local_offset_idx'].long() - LOCAL_START_OFFSET_RADIUS_BINS
    count_condition_mae = torch.abs(batch['predicted_count_norm'] - batch.get('target_count_norm', batch['predicted_count_norm'])).mean().item()
    return {
        'start_exact_acc': (pred_start == target_start).float().mean().item() * 100.0,
        'day_offset_acc': (pred_day_offset == true_day_offset).float().mean().item() * 100.0,
        'local_offset_acc': (pred_local_offset == true_local_offset).float().mean().item() * 100.0,
        'start_tol_acc': (diff <= START_TOPK_TOLERANCE_BINS).float().mean().item() * 100.0,
        'start_tol_acc_5m': (diff <= 1).float().mean().item() * 100.0,
        'start_tol_acc_10m': (diff <= 2).float().mean().item() * 100.0,
        'start_mae_minutes': diff.float().mean().item() * BIN_MINUTES,
        'duration_mae_minutes': torch.abs(outputs['pred_duration_norm'] - batch['target_duration_norm']).mean().item() * duration_span_minutes,
        'count_condition_mae_norm': count_condition_mae,
    }
