...
from data.io import DEFAULT_DEVICE_UID, load_tasks_dataframe
...
def _task_duration_median(prepared, task_id: int) -> float:
    mask = prepared.df['task_id'] == int(task_id)
    if not mask.any():
...
def _repair_predictions(raw_predictions: list[dict]) -> list[dict]:
    placed_by_device: dict[str, list[tuple[int, int]]] = {}
    ...
    for item in sorted(raw_predictions, key=lambda x: (x['preferred_start_bin'], x['task_id'], x['occurrence_index'])):
        ...
        device_uid = str(item['device_uid'])
        placed = placed_by_device.setdefault(device_uid, [])
    ...
    with torch.no_grad():
        ...
        for task_id, count in enumerate(pred_counts):
            ...
            for occ_idx in range(count):
                ...
                denom = max(count - 1, 1)
                occurrence_progresses.append(float(occ_idx / denom) if count > 1 else 0.0)
        ...
        for i in range(batch_size):
            task_name = prepared.task_base_names[task_ids[i]]
            device_uid = prepared.task_device_uids[task_ids[i]]
            pred_duration = denormalize_duration(
                float(outputs['pred_duration_norm'][i].item()),
                prepared.duration_min,
                prepared.duration_max,
            )
            median_duration = _task_duration_median(prepared, task_ids[i])
            duration = (
...
def _extract_preprocessing_caps(occ_ckpt: dict, tmp_ckpt: dict, df: pd.DataFrame | None = None) -> tuple[int, int]:
    ...
    if df is not None and not df.empty:
        ...
        if 'device_uid' not in df_for_caps.columns:
            df_for_caps['device_uid'] = DEFAULT_DEVICE_UID
        df_for_caps['series_name'] = [
            f'{device_uid}::{task_name}'
            for device_uid, task_name in zip(df_for_caps['device_uid'].astype(str).tolist(), df_for_caps['task_name'].astype(str).tolist())
        ]
        task_names = sorted(df_for_caps['series_name'].unique().tolist())
        task_to_id = {name: idx for idx, name in enumerate(task_names)}
        df_for_caps['task_id'] = df_for_caps['series_name'].map(task_to_id)
...
def _materialize_prediction_times(prepared, predictions: list[dict]) -> list[dict]:
    ...
    for item in predictions:
        ...
        if device_uid == DEFAULT_DEVICE_UID:
            device_uid = None
