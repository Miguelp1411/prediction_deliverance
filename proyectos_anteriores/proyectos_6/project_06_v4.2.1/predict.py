...
def predict_next_week(occurrence_model, temporal_model, prepared, target_week_idx, device):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            candidate_starts = _merge_candidate_scores(
                model_candidates,
                anchor_candidates_list[i],
                anchor_candidate_weights_list[i],
            ...
            predictions.append({
                ...
                'start_bin': int(candidate_starts[0][0] if candidate_starts else anchor_start_bins[i]),
                'duration': float(duration),
                'candidate_starts': candidate_starts,
            })

    reranked = _beam_rerank_weekly(predictions)
    if PREDICTION_USE_REPAIR:
        reranked = _repair_predictions(reranked)
    return sorted(reranked, key=lambda x: (x['start_bin'], x['task_name']))
