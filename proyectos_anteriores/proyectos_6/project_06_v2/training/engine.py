...
from proyectos_anteriores.proyectos_6.project_06_v2.config import TEMPORAL_COUNT_NOISE_STD, VERBOSE_EVERY
...
def _optimizer_lr_summary(optimizer) -> str:
    ...
    for group in optimizer.param_groups:
        lr = group.get('lr')
        if lr is not None:
            lrs.append(f'{float(lr):.2e}')
    return ','.join(lrs) if lrs else '-'
...
def fit_model(
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        improved_this_epoch = _is_improvement(monitor_value, state.best_metric, monitor_mode, min_delta)
        ...
        state.history[-1].update({
            ...
            'improved': bool(improved_this_epoch),
            'learning_rates': [float(group.get('lr', 0.0)) for group in optimizer.param_groups],
        })

        if epoch == 1 or epoch % VERBOSE_EVERY == 0 or improved_this_epoch:
            status = 'IMPROVED' if improved_this_epoch else f'sin_mejora={state.epochs_without_improvement}/{patience}'
            print(f'\n[{model_name}] Epoch {epoch}')
            print(
                '  '
                f'train_loss={train_loss:.4f} | '
                f'val_loss={val_loss:.4f} | '
                f'{state.monitor_name}={monitor_value:.4f}'
            )
            print(
                '  '
                f'best_{state.monitor_name}={state.best_metric:.4f} '
                f'(epoch {state.best_epoch}) | '
                f'{status} | '
                f'lr={_optimizer_lr_summary(optimizer)}'
            )
            if train_metrics:
