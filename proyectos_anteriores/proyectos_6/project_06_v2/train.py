...
from proyectos_anteriores.proyectos_6.project_06_v2.config import (
    CAP_INFERENCE_SCOPE,
    CHECKPOINT_DIR,
    DATA_PATH,
    DAY_EMBED_DIM,
...
from proyectos_anteriores.proyectos_6.project_06_v2.data.datasets import OccurrenceDataset, TemporalDataset, build_occurrence_count_lookup, build_split_indices
from proyectos_anteriores.proyectos_6.project_06_v2.data.io import load_tasks_dataframe
from proyectos_anteriores.proyectos_6.project_06_v2.data.preprocessing import prepare_data, serialize_metadata
from proyectos_anteriores.proyectos_6.project_06_v2.evaluation.weekly_stats import evaluate_weekly_predictions
from proyectos_anteriores.proyectos_6.project_06_v2.models.occurrence_model import StructuredOccurrenceModel, TaskOccurrenceModel
from proyectos_anteriores.proyectos_6.project_06_v2.models.temporal_model import TemporalAssignmentModel
from proyectos_anteriores.proyectos_6.project_06_v2.predict import predict_next_week
from proyectos_anteriores.proyectos_6.project_06_v2.training.engine import evaluate_epoch, fit_model
from proyectos_anteriores.proyectos_6.project_06_v2.training.losses import TemporalLoss
from proyectos_anteriores.proyectos_6.project_06_v2.training.metrics import occurrence_metrics, temporal_metrics
from proyectos_anteriores.proyectos_6.project_06_v2.utils.serialization import save_checkpoint
...
def print_temporal_monitor_guide():
    print('\nGuía del monitor temporal')
    print('-' * 50)
    print('  Monitor principal : e2e_joint_score (max)')
    print('  Fórmula           : e2e_task_acc + e2e_start_exact_acc - 5 * e2e_overlap_count')
    print(f'  Early stopping    : patience={TMP_PATIENCE} | min_delta=0.10 | max_epochs={TMP_MAX_EPOCHS}')
    print(f'  Scheduler         : ReduceLROnPlateau(mode=max, factor=0.5, patience={TMP_SCHEDULER_PATIENCE})')
    print('  Pérdida optimizada: 0.80*day_ce + 1.00*time_ce + 0.20*duration_smooth_l1')
    print('  overlap_count     : solapes dentro de la semana predicha, no en la base histórica')
    print('  count_condition_mae_norm:')
    print('    error medio entre el conteo usado como contexto por el TemporalModel y el conteo real del target')


def save_training_report(path, model_name: str, state, extra: dict | None = None):
    path.parent.mkdir(parents=True, exist_ok=True)
    best_train_metrics = getattr(state, 'best_train_metrics', {}) or {}
    best_val_metrics = getattr(state, 'best_val_metrics', {}) or {}
    final_train_metrics = getattr(state, 'final_train_metrics', {}) or {}
    final_val_metrics = getattr(state, 'final_val_metrics', {}) or {}
    payload = {
        'model_name': model_name,
        'monitor_name': getattr(state, 'monitor_name', 'unknown'),
        'monitor_mode': getattr(state, 'monitor_mode', 'unknown'),
        'best_epoch': int(getattr(state, 'best_epoch', 0)),
        'best_metric': float(getattr(state, 'best_metric', float('nan'))),
        'best_train_loss': float(getattr(state, 'best_train_loss', float('nan'))),
        'best_val_loss': float(getattr(state, 'best_val_loss', float('nan'))),
        'final_train_loss': float(getattr(state, 'final_train_loss', float('nan'))),
        'final_val_loss': float(getattr(state, 'final_val_loss', float('nan'))),
        'best_train_metrics': _to_float_dict(best_train_metrics),
        'best_val_metrics': _to_float_dict(best_val_metrics),
        'final_train_metrics': _to_float_dict(final_train_metrics),
        'final_val_metrics': _to_float_dict(final_val_metrics),
...
def summarize_weekly_predictions(label, stats: dict):
    ...
    print(f"E2E joint score     : {stats['e2e_joint_score']:.2f}")
...
def main():
    ...
    save_training_report(
        CHECKPOINT_DIR / 'occurrence_training_report.json',
        'OccurrenceModel',
        occ_state,
        extra={
            ...
            'occurrence_lag_weeks': int(OCC_LAG_WEEKS),
        },
    )
    save_training_report(
        CHECKPOINT_DIR / 'temporal_training_report.json',
        'TemporalModel',
        tmp_state,
        extra={
            'data_path': str(DATA_PATH),
            'train_ratio': float(TRAIN_RATIO),
            'device': str(device),
            'monitor_formula': 'e2e_task_acc + e2e_start_exact_acc - 5 * e2e_overlap_count',
            'count_blend_alpha_train': float(TEMPORAL_COUNT_BLEND_TARGET_WEIGHT),
            'count_blend_alpha_val': 0.0,
            'tmp_lr': float(TMP_LR),
            'tmp_weight_decay': float(TMP_WEIGHT_DECAY),
            'tmp_batch_size': int(TMP_BATCH_SIZE),
            'tmp_max_epochs': int(TMP_MAX_EPOCHS),
            'tmp_patience': int(TMP_PATIENCE),
            'tmp_scheduler_patience': int(TMP_SCHEDULER_PATIENCE),
        },
    )
    ...
    print(f"  - {CHECKPOINT_DIR / 'occurrence_training_report.json'}")
    print(f"  - {CHECKPOINT_DIR / 'temporal_training_report.json'}")
