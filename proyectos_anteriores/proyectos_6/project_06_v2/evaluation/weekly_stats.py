...
from proyectos_anteriores.proyectos_6.project_06_v2.evaluation.matching import hungarian_match
...
def _normalize_true(true_tasks):
    if hasattr(true_tasks, 'events_by_task'):
        ...
        for events in true_tasks.events_by_task.values():
            for e in events:
                out.append({'task_name': _event_label(e), 'start_bin': int(e.start_bin), 'duration': float(e.duration_minutes)})
        return out
    return [{'task_name': str(item.get('task_name', item.get('type'))), 'start_bin': int(item['start_bin']), 'duration': float(item['duration'])} for item in true_tasks]


def _normalize_pred(pred_tasks):
    return [{'task_name': _pred_label(item), 'start_bin': int(item['start_bin']), 'duration': float(item['duration'])} for item in pred_tasks]
...
def _count_overlaps(pred_tasks: list[dict[str, Any]]) -> int:
    ...
    for item in pred_tasks:
        ...
        intervals.append((start, start + duration_bins))
    intervals.sort()
    overlaps = 0
    prev_end = None
