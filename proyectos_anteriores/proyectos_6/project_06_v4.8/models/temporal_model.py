...
from config import bins_per_day, num_day_classes, num_time_of_day_classes
...
class TemporalAssignmentModel(nn.Module):
    def __init__(
        ...
        self.anchor_time_embedding = nn.Embedding(num_time_of_day_classes(), day_embed_dim)
        ...
        fused_dim = hidden_size * 2 + task_embed_dim + occurrence_embed_dim + day_embed_dim * 2 + hidden_size
        ...
        self.time_of_day_head = nn.Linear(hidden_size * 2, num_time_of_day_classes())
    ...
    def forward_with_context(
        ...
        anchor_time_vec = self.anchor_time_embedding(anchor_time_bin.clamp(min=0, max=bins_per_day() - 1))
        ...
        fused = self.fusion(torch.cat([sequence_context, task_vec, occ_vec, anchor_day_vec, anchor_time_vec, hist_vec], dim=-1))
        return {
            'day_logits': self.day_head(fused),
            'time_of_day_logits': self.time_of_day_head(fused),
