...
from config import (
    ...
    TEMPORAL_VALIDATION_SCORE_EXACT_WEIGHT,
    TEMPORAL_VALIDATION_SCORE_MAE_WEIGHT,
    TEMPORAL_VALIDATION_SCORE_OVERLAP_WEIGHT,
    TEMPORAL_VALIDATION_SCORE_TOL10_WEIGHT,
    TEMPORAL_VALIDATION_SCORE_TOL5_WEIGHT,
    TEMPORAL_VALIDATION_STAGE_CANDIDATES,
    REPORTS_DIR,
    SEED,
    TASK_EMBED_DIM,
...
from models.occurrence_model import StructuredOccurrenceModel, TaskOccurrenceModel, TaskMoEOccurrenceModelV48
from models.temporal_model import TemporalAssignmentModel
from predict import load_runtime_overrides, predict_next_week, predict_next_week_staged
...
TEMPORAL_REPORT_KEYS = (
    ...
    'start_tol_acc_5m',
    'start_tol_acc_10m',
    'start_bucket_0_5m_only',
    'start_bucket_5_10m_only',
    'start_mae_minutes',
...
ENSEMBLE_REPORT_KEYS = (
    ...
    'time_bucket_0_5m_only',
    'time_bucket_5_10m_only',
...
TEMPORAL_LABELS = {
    ...
    'start_tol_acc_5m': 'start_acc_≤5m (incl exact)',
    'start_tol_acc_10m': 'start_acc_≤10m (incl ≤5m)',
    'start_bucket_0_5m_only': 'start_bucket_(0,5]m_only',
    'start_bucket_5_10m_only': 'start_bucket_(5,10]m_only',
...
ENSEMBLE_LABELS = {
    ...
    'task_f1': 'task_f1',
    'time_exact_accuracy': 'time_exact_accuracy',
    'time_close_accuracy_5m': 'time_acc_≤5m (incl exact)',
    'time_close_accuracy_10m': 'time_acc_≤10m (incl ≤5m)',
    'time_bucket_0_5m_only': 'time_bucket_(0,5]m_only',
    'time_bucket_5_10m_only': 'time_bucket_(5,10]m_only',
    'duration_close_accuracy': 'duration_close_accuracy',
    'start_mae_minutes': 'start_mae_minutes',
    'overlap_same_device_count': 'overlap_same_device_count',
    'overlap_global_count': 'overlap_global_count',
...
ABLATION_STAGE_ORDER = tuple(TEMPORAL_VALIDATION_STAGE_CANDIDATES)
...
def _finalize_weekly_totals(
    ...
    result = {
        **totals,
        ...
        'time_accuracy_upto_5m': totals['time_close_5m_count'] / total_tasks if total_tasks > 0 else 0.0,
        'time_accuracy_upto_10m': totals['time_close_10m_count'] / total_tasks if total_tasks > 0 else 0.0,
        'time_bucket_0_5m_only': max(totals['time_close_5m_count'] - totals['time_exact_count'], 0.0) / total_tasks if total_tasks > 0 else 0.0,
        'time_bucket_5_10m_only': max(totals['time_close_10m_count'] - totals['time_close_5m_count'], 0.0) / total_tasks if total_tasks > 0 else 0.0,
...
def print_final_report(report: dict):
    ...
    for key, value in temporal['metrics'].items():
        label = TEMPORAL_LABELS.get(key, key)
        unit = ' min' if 'mae_minutes' in key else ('%' if 'acc' in key or 'bucket' in key else '')
        print(f'  {label:<26} {value:.4f}{unit}')

    ensemble = report['ensemble']
    print("\nEnsamblado final")
    for key, value in ensemble['metrics'].items():
        label = ENSEMBLE_LABELS.get(key, key)
        if key in PERCENTAGE_REPORT_KEYS:
            print(f'  {label:<26} {value:.4f}%')
        elif key == 'start_mae_minutes':
            print(f'  {label:<26} {value:.4f} min')
        else:
            print(f'  {label:<26} {value:.4f}')
    ...
    if ablation.get('stage_order'):
        ...
        for stage in ablation['stage_order']:
            ...
            tol10 = metrics.get('time_close_accuracy_10m', 0.0)
            mae = metrics.get('start_mae_minutes', 0.0)
            print(f"  {stage:<8} exact={exact:.2f}% | ≤5m(acum)={tol5:.2f}% | ≤10m(acum)={tol10:.2f}% | mae={mae:.2f} min")
        selection = ablation.get('selection', {})
        if selection.get('selected_stage'):
            print(
                f"  stage_select -> {selection.get('selected_stage')} "
                f"(score={selection.get('selected_stage_score', 0.0):.4f})"
            )
...
def aggregate_weekly_stats(prepared, week_indices, occurrence_model, temporal_model, device, ensemble_stage: str = 'final'):
    if not week_indices:
        return {}
    ...
    for idx in week_indices:
        pred_week = predict_next_week(occurrence_model, temporal_model, prepared, idx, device, ensemble_stage=ensemble_stage)
...
def aggregate_weekly_ablation_stats(prepared, week_indices, occurrence_model, temporal_model, device, include_repair: bool = False, include_gated: bool = True):
    if not week_indices:
        return {'stage_order': [], 'stages': {}, 'transitions': {}}

    stage_order = [stage for stage in ABLATION_STAGE_ORDER if include_gated or stage != 'gated']
    if include_repair and 'gated' in stage_order:
        stage_order = [stage for stage in stage_order if stage != 'gated']
    if include_repair:
    ...
    for idx in week_indices:
        staged_predictions = predict_next_week_staged(
            ...
            requested_stage='repair' if include_repair else ('gated' if include_gated else 'rerank'),
        )
...
class TemporalE2EEvaluator:
    ...
    def __init__(self, prepared, week_indices, occurrence_model, device, ensemble_stage: str = 'final'):
        self.prepared = prepared
        self.week_indices = list(week_indices)
        self.occurrence_model = occurrence_model
        self.device = device
        self.ensemble_stage = str(ensemble_stage)

    @torch.no_grad()
    def __call__(self, temporal_model):
        temporal_model.eval()
        stats = aggregate_weekly_stats(self.prepared, self.week_indices, self.occurrence_model, temporal_model, self.device, ensemble_stage=self.ensemble_stage)
        compound_score = (
        ...
        enriched['e2e_time_close_accuracy_5m'] = float(stats.get('time_close_accuracy_5m', 0.0))
        enriched['e2e_start_mae_minutes'] = float(stats.get('start_mae_minutes', 0.0))
        return {
            ...
            for k in [
                ...
                'e2e_joint_score',
                'e2e_compound_score',
                'e2e_time_close_accuracy_5m',
                'e2e_start_mae_minutes',
            ]
        }


def stage_objective_from_metrics(metrics: dict[str, float]) -> float:
    tol5 = float(metrics.get('time_close_accuracy_5m', 0.0))
    exact = float(metrics.get('time_exact_accuracy', 0.0))
    tol10 = float(metrics.get('time_close_accuracy_10m', 0.0))
    mae = float(metrics.get('start_mae_minutes', 0.0))
    overlaps = float(metrics.get('overlap_same_device_count', 0.0))
    return (
        TEMPORAL_VALIDATION_SCORE_TOL5_WEIGHT * tol5
        + TEMPORAL_VALIDATION_SCORE_EXACT_WEIGHT * exact
        + TEMPORAL_VALIDATION_SCORE_TOL10_WEIGHT * tol10
        - TEMPORAL_VALIDATION_SCORE_MAE_WEIGHT * mae
        - TEMPORAL_VALIDATION_SCORE_OVERLAP_WEIGHT * overlaps
    )


def select_best_validation_stage(ablation_stats: dict) -> dict[str, object]:
    stage_scores: dict[str, float] = {}
    for stage in ablation_stats.get('stage_order', []):
        metrics = dict(ablation_stats.get('stages', {}).get(stage, {}))
        if not metrics:
            continue
        stage_scores[stage] = stage_objective_from_metrics(metrics)
    if not stage_scores:
        return {'selected_stage': 'raw', 'selected_stage_score': float('-inf'), 'stage_scores': {}}
    selected_stage, selected_score = max(stage_scores.items(), key=lambda item: item[1])
    return {
        'selected_stage': str(selected_stage),
        'selected_stage_score': _round_float(selected_score),
        'stage_scores': {str(k): _round_float(v) for k, v in stage_scores.items()},
    }
...
def _build_ablation_report(ablation_stats: dict) -> dict:
    ...
    selection = select_best_validation_stage(ablation_stats)
    return {
...
def build_final_report(occ_state, occ_val_metrics, tmp_state, tmp_val_metrics, ensemble_stats, ablation_stats, report_path: Path) -> dict:
    ...
    return {
        ...
        'meta': {
            ...
            'temporal_final_stage': TEMPORAL_FINAL_STAGE,
            'temporal_e2e_stage': TEMPORAL_E2E_STAGE,
            'temporal_gating_enable': bool(TEMPORAL_GATING_ENABLE),
            'temporal_rerank_only_on_conflict': bool(TEMPORAL_RERANK_ONLY_ON_CONFLICT),
            'validation_stage_selection': select_best_validation_stage(ablation_stats),
        },
...
def build_occurrence_model(prepared, occ_train: OccurrenceDataset, device: torch.device):
    ...
    if OCCURRENCE_MODEL_KIND == 'task_moe_v48':
        task_delta_bounds = _compute_task_delta_bounds(prepared, list(occ_train.week_indices))
        model = TaskMoEOccurrenceModelV48(
            prepared.week_feature_dim,
            len(prepared.task_names),
            prepared.max_count_cap,
            OCC_HIDDEN_SIZE,
        ...
        if len(occ_train) > 0:
            target_counts = torch.as_tensor(np.stack([occ_train.targets[i] for i in range(len(occ_train))]), dtype=torch.long, device=device)
        else:
            target_counts = None
        model.fit(target_counts)
        return model, None
...
def main():
    ...
    set_seed(SEED)
    applied_overrides = load_runtime_overrides(POSTPROCESS_OVERRIDE_PATH)
    device = resolve_device(DEVICE)
    configure_runtime(device)
    print(f"\nDevice de entrenamiento: {device}")
    if applied_overrides:
        print(f"Overrides de postproceso cargados: {sorted(applied_overrides.keys())}")
    if device.type == 'cuda':
    ...
    e2e_evaluator = TemporalE2EEvaluator(prepared, split.val_target_week_indices, occurrence_model, device, ensemble_stage=TEMPORAL_E2E_STAGE)
    ...
    ensemble_val_stats = aggregate_weekly_stats(
        ...
        temporal_model,
        device,
        ensemble_stage=TEMPORAL_FINAL_STAGE,
    )
    ...
    validation_stage_selection = select_best_validation_stage(ensemble_ablation_stats)
    ...
    metadata['selected_final_stage_validation'] = str(validation_stage_selection.get('selected_stage', 'raw'))
    metadata['selected_final_stage_score_validation'] = float(validation_stage_selection.get('selected_stage_score', float('-inf')) )
    metadata['validation_stage_scores'] = dict(validation_stage_selection.get('stage_scores', {}))
