...
class UnifiedWeekPredictor:
    ...
    def predict_database_robot(self, database_id: str, robot_id: str) -> dict:
        ...
        anchor_durations = np.maximum(1, np.round(np.expm1(batch['target_log_duration'][0].cpu().numpy())).astype(int))
        ...
        max_selected_slots_per_task = int(self.config['inference'].get('max_selected_slots_per_task', self.max_slots))

        for task_idx in range(len(self.context.task_names)):
            indices = np.where(task_ids == task_idx)[0].tolist()
            if not indices:
                continue
            probs = np.asarray([active_probs[i] for i in indices], dtype=np.float32)
            expected_count = int(np.clip(np.rint(probs.sum()), 0, min(len(indices), max_selected_slots_per_task)))
            ranked = sorted(indices, key=lambda i: float(active_probs[i]), reverse=True)
            chosen = [i for i in ranked if float(active_probs[i]) >= threshold][:max_selected_slots_per_task]
