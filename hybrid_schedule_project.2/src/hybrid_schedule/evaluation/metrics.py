...
@torch.no_grad()
def temporal_metrics(outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], day_radius: int, time_radius_bins: int, bins_per_day: int, bin_minutes: int) -> dict[str, float]:
    pred_day = outputs['day_logits'].argmax(dim=-1) - day_radius
    pred_time = outputs['time_logits'].argmax(dim=-1) - time_radius_bins
    pred_start = batch['anchor_start'] + pred_day * bins_per_day + pred_time
