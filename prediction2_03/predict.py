...
def _search_nearest_within_bounds(
    preferred_start: int,
    duration_bins: int,
...
def _same_day_or_adjacent_valid_start(
    preferred_start: int,
    duration_bins: int,
    placed_intervals: list[tuple[int, int]],
    lower_bound: int = 0,
...
def _repair_predictions(raw_predictions: list[dict]) -> list[dict]:
    ...
    for item in sorted(raw_predictions, key=lambda x: (x['preferred_start_bin'], x['task_id'], x['occurrence_index'])):
        ...
        preferred_start = int(item['preferred_start_bin'])
        lower_bound = min(last_start_by_task.get(item['task_id'], 0), preferred_start)
        preferred_day = preferred_start // bins_per_day()
        candidate_pool: list[tuple[tuple[int, int, int], int]] = []
        ...
        for candidate_start, _ in item['candidate_starts']:
            ...
            if _overlaps(candidate_start, duration_bins, placed):
                continue
            candidate_pool.append((_candidate_cost(candidate_start, preferred_start), candidate_start))

        if candidate_pool:
            candidate_pool.sort(key=lambda x: x[0])
            chosen_start = int(candidate_pool[0][1])
        else:
            chosen_start = _same_day_or_adjacent_valid_start(
                preferred_start,
                duration_bins,
                placed,
            ...
            if chosen_start is None and PREDICTION_REPAIR_GLOBAL_FALLBACK:
                chosen_start = _best_global_valid_start(
                    preferred_start,
                    duration_bins,
                    placed,
                    lower_bound=lower_bound,
        ...
        final_predictions.append({
            ...
            'repair_day_shift': int(chosen_start // bins_per_day()) - int(preferred_day),
...
def predict_next_week(occurrence_model, temporal_model, prepared, target_week_idx, device, use_repair: bool = False, auxiliary_corrector=None, aux_correct_count: bool = True, aux_correct_time: bool = False, aux_correct_duration: bool = True):
    ...
    if use_repair:
        final_predictions = _repair_predictions(raw_predictions)
    else:
        final_predictions = []
        ...
        final_predictions = sorted(final_predictions, key=lambda x: (x['start_bin'], x['task_name']))

    if auxiliary_corrector is not None:
        final_predictions = auxiliary_corrector.apply(
            prepared,
            target_week_idx,
            final_predictions,
            correct_count=aux_correct_count,
            correct_time=aux_correct_time,
            correct_duration=aux_correct_duration,
            apply_repair=use_repair,
        )
...
def main():
    ...
    parser.add_argument('--disable-auxiliary-corrector', action='store_true', help='Desactiva la carga y aplicación del corrector auxiliar.')
    parser.add_argument('--auxiliary-checkpoint', type=str, default=None, help='Ruta opcional al checkpoint del corrector auxiliar.')
    parser.add_argument('--disable-aux-count', action='store_true', help='Desactiva la corrección auxiliar de conteo.')
    parser.add_argument('--enable-aux-count', action='store_true', help='Fuerza la corrección auxiliar de conteo aunque la policy la desactive.')
    parser.add_argument('--disable-aux-time', action='store_true', help='Desactiva la corrección auxiliar temporal fina aunque la policy la active.')
    parser.add_argument('--enable-aux-time', action='store_true', help='Activa la corrección auxiliar temporal fina.')
    parser.add_argument('--disable-aux-duration', action='store_true', help='Desactiva la corrección auxiliar de duración.')
    parser.add_argument('--enable-aux-duration', action='store_true', help='Fuerza la corrección auxiliar de duración aunque la policy la desactive.')
    parser.add_argument('--ignore-aux-policy', action='store_true', help='Ignora la policy automática y usa solo los flags CLI.')
    ...
    use_aux_count = not args.disable_aux_count
    use_aux_time = args.enable_aux_time
    use_aux_duration = not args.disable_aux_duration

    if not args.disable_auxiliary_corrector:
        from auxiliary_corrector import maybe_load_auxiliary_corrector, maybe_load_auxiliary_policy
        policy = None if args.ignore_aux_policy else maybe_load_auxiliary_policy()
        if policy is not None:
            use_aux_count = bool(policy.use_aux_count)
            use_aux_time = bool(policy.use_aux_time)
            use_aux_duration = bool(policy.use_aux_duration)
            if not bool(policy.use_auxiliary):
                print('Policy automática: el corrector auxiliar no mejoró validación y no se aplicará en predicción.')
            else:
                print(
                    'Policy automática cargada: '
                    f'count={policy.use_aux_count}, duration={policy.use_aux_duration}, time={policy.use_aux_time}'
                )
        if args.enable_aux_count:
            use_aux_count = True
        if args.disable_aux_count:
            use_aux_count = False
        if args.enable_aux_duration:
            use_aux_duration = True
        if args.disable_aux_duration:
            use_aux_duration = False
        if args.enable_aux_time:
            use_aux_time = True
        if args.disable_aux_time:
            use_aux_time = False

        if (policy is None or policy.use_auxiliary or args.ignore_aux_policy or args.enable_aux_count or args.enable_aux_duration or args.enable_aux_time) and (use_aux_count or use_aux_time or use_aux_duration):
            auxiliary_corrector = maybe_load_auxiliary_corrector(args.auxiliary_checkpoint)
            if auxiliary_corrector is None:
                print('Aviso: no se encontró un corrector auxiliar compatible; se usará solo el modelo principal.')
                use_aux_count = False
                use_aux_time = False
                use_aux_duration = False
    ...
    print(f'Auxiliar aplicado: {bool(auxiliary_corrector is not None and (use_aux_count or use_aux_duration or use_aux_time))} (count={use_aux_count}, duration={use_aux_duration}, time={use_aux_time})')
