...
from auxiliary_corrector import (
...
def _average_metrics(stats_list):
    if not stats_list:
        return {}
    keys = [
        'task_accuracy',
    ...
    return {key: float(np.mean([float(s.get(key, 0.0)) for s in stats_list])) for key in keys}


def _evaluate_auxiliary_variant(
    week_indices,
    prepared,
    occurrence_model,
    temporal_model,
    ...
    *,
    ...
    for idx in week_indices:
        pred = predict_next_week(
            ...
            aux_correct_count=use_aux_count,
            aux_correct_time=use_aux_time,
            aux_correct_duration=use_aux_duration,
        )
        stats.append(evaluate_weekly_predictions(prepared.weeks[idx], pred))
    return _average_metrics(stats)


def _accept_count(candidate: dict[str, float], baseline: dict[str, float]) -> bool:
    eps = 0.002
    return (
        candidate.get('task_accuracy', 0.0) >= baseline.get('task_accuracy', 0.0) + eps
        and candidate.get('day_exact_accuracy', 0.0) >= baseline.get('day_exact_accuracy', 0.0) - 0.01
        and candidate.get('time_exact_accuracy', 0.0) >= baseline.get('time_exact_accuracy', 0.0) - 0.01
    )


def _accept_duration(candidate: dict[str, float], baseline: dict[str, float]) -> bool:
    eps = 0.002
    return (
        candidate.get('duration_close_accuracy', 0.0) >= baseline.get('duration_close_accuracy', 0.0) + eps
        and candidate.get('task_accuracy', 0.0) >= baseline.get('task_accuracy', 0.0) - 0.005
        and candidate.get('time_exact_accuracy', 0.0) >= baseline.get('time_exact_accuracy', 0.0) - 0.01
    )


def _accept_time(candidate: dict[str, float], baseline: dict[str, float]) -> bool:
    eps = 0.002
    mae_gain = 5.0
    time_exact_gain = candidate.get('time_exact_accuracy', 0.0) - baseline.get('time_exact_accuracy', 0.0)
    time5_gain = candidate.get('time_close_accuracy_5m', 0.0) - baseline.get('time_close_accuracy_5m', 0.0)
    mae_delta = baseline.get('start_mae_minutes', 0.0) - candidate.get('start_mae_minutes', 0.0)
    return (
        max(time_exact_gain, time5_gain) >= eps
        and mae_delta >= mae_gain
        and candidate.get('day_exact_accuracy', 0.0) >= baseline.get('day_exact_accuracy', 0.0) - 0.005
    )


def derive_auxiliary_policy(
    week_indices,
    prepared,
    occurrence_model,
    temporal_model,
    ...
    base_metrics = _evaluate_auxiliary_variant(
        week_indices,
        prepared,
        occurrence_model,
        temporal_model,
        ...
        device=device,
        use_aux_count=False,
        use_aux_duration=False,
        use_aux_time=False,
    )

    selected_flags = {'count': False, 'duration': False, 'time': False}
    current_metrics = dict(base_metrics)
    audit: dict[str, dict[str, float] | bool] = {'base': dict(base_metrics)}

    for module in ('count', 'duration', 'time'):
        flags = dict(selected_flags)
        flags[module] = True
        candidate_metrics = _evaluate_auxiliary_variant(
            week_indices,
            prepared,
            occurrence_model,
            temporal_model,
            auxiliary_corrector=auxiliary_corrector,
            device=device,
            use_aux_count=flags['count'],
            use_aux_duration=flags['duration'],
            use_aux_time=flags['time'],
        )
        audit[f'candidate_{module}'] = dict(candidate_metrics)
        if module == 'count':
            accepted = _accept_count(candidate_metrics, current_metrics)
        elif module == 'duration':
            accepted = _accept_duration(candidate_metrics, current_metrics)
        else:
            accepted = _accept_time(candidate_metrics, current_metrics)
        audit[f'accepted_{module}'] = bool(accepted)
        if accepted:
            selected_flags[module] = True
            current_metrics = dict(candidate_metrics)

    policy = AuxiliaryUsagePolicy(
        use_auxiliary=bool(selected_flags['count'] or selected_flags['duration'] or selected_flags['time']),
        use_aux_count=bool(selected_flags['count']),
        use_aux_duration=bool(selected_flags['duration']),
        use_aux_time=bool(selected_flags['time']),
        metrics={
...
def summarize_auxiliary_corrector(
    label,
    week_indices,
    prepared,
    occurrence_model,
    ...
    if not week_indices:
        print(f'\nResumen corrector auxiliar — {label}: sin semanas disponibles.')
        return

    base_stats, aux_cd_stats, aux_full_stats = [], [], []
    for idx in week_indices:
        ...
        aux_cd_pred = predict_next_week(
            ...
            aux_correct_time=False,
            aux_correct_duration=True,
        )
        aux_full_pred = predict_next_week(
            occurrence_model,
            temporal_model,
            prepared,
            idx,
            ...
            auxiliary_corrector=auxiliary_corrector,
            aux_correct_count=True,
            aux_correct_time=True,
            aux_correct_duration=True,
        )
        truth = prepared.weeks[idx]
        base_stats.append(evaluate_weekly_predictions(truth, base_pred))
        aux_cd_stats.append(evaluate_weekly_predictions(truth, aux_cd_pred))
        aux_full_stats.append(evaluate_weekly_predictions(truth, aux_full_pred))

    def avg(stats_list, key):
        return float(np.mean([s[key] for s in stats_list])) if stats_list else 0.0

    print(f'\nResumen corrector auxiliar — {label}')
    print('-' * 72)
    print(f"Semanas evaluadas             : {len(week_indices)}")
    print(f"task_acc base -> c+d -> full  : {avg(base_stats, 'task_accuracy')*100:.1f}% -> {avg(aux_cd_stats, 'task_accuracy')*100:.1f}% -> {avg(aux_full_stats, 'task_accuracy')*100:.1f}%")
    print(f"día exacto base -> c+d -> full: {avg(base_stats, 'day_exact_accuracy')*100:.1f}% -> {avg(aux_cd_stats, 'day_exact_accuracy')*100:.1f}% -> {avg(aux_full_stats, 'day_exact_accuracy')*100:.1f}%")
    print(f"hora exacta base -> c+d -> ful: {avg(base_stats, 'time_exact_accuracy')*100:.1f}% -> {avg(aux_cd_stats, 'time_exact_accuracy')*100:.1f}% -> {avg(aux_full_stats, 'time_exact_accuracy')*100:.1f}%")
    print(f"hora ±5m base -> c+d -> full  : {avg(base_stats, 'time_close_accuracy_5m')*100:.1f}% -> {avg(aux_cd_stats, 'time_close_accuracy_5m')*100:.1f}% -> {avg(aux_full_stats, 'time_close_accuracy_5m')*100:.1f}%")
    print(f"MAE inicio base -> c+d -> ful : {avg(base_stats, 'start_mae_minutes'):.2f} -> {avg(aux_cd_stats, 'start_mae_minutes'):.2f} -> {avg(aux_full_stats, 'start_mae_minutes'):.2f} min")
    print(f"dur ±2m base -> c+d -> full   : {avg(base_stats, 'duration_close_accuracy')*100:.1f}% -> {avg(aux_cd_stats, 'duration_close_accuracy')*100:.1f}% -> {avg(aux_full_stats, 'duration_close_accuracy')*100:.1f}%")
...
def summarize_weekly_predictions(
    ...
    day_exact_accuracy = float(avg.get('day_exact_accuracy', avg.get('day_exact_acc', 0.0) / 100.0))
    day_close_accuracy_1d = float(avg.get('day_close_accuracy_1d', avg.get('day_tol_acc_1d', 0.0) / 100.0))
    start_mae_same_day = float(avg.get('start_mae_when_day_correct_minutes', 0.0))
    ...
    print(f"Día exacto          : {day_exact_accuracy * 100:.1f}%")
    print(f"Día ±1              : {day_close_accuracy_1d * 100:.1f}%")
    print(f"MAE inicio          : {start_mae_minutes:.2f} min")
    print(f"MAE inicio | día ok : {start_mae_same_day:.2f} min")
...
def main():
    parser = argparse.ArgumentParser(description='Entrena OccurrenceModel, TemporalAssignmentModel y el corrector auxiliar residual.')
    parser.add_argument('--data', type=str, default=str(DATA_PATH), help='Ruta al JSON de datos históricos.')
    parser.add_argument('--skip-auxiliary-corrector', action='store_true', help='Omite el entrenamiento del corrector auxiliar.')
    ...
    if not args.skip_auxiliary_corrector:
        try:
            aux_train = AuxiliaryCorrector.fit_from_history(
                prepared,
                occurrence_model,
                temporal_model,
                device,
                week_indices=split.train_target_week_indices,
            )
            summarize_auxiliary_corrector(
                'validación',
                split.val_target_week_indices[:8],
                prepared,
                occurrence_model,
            ...
            policy = derive_auxiliary_policy(
                split.val_target_week_indices,
                prepared,
                occurrence_model,
                temporal_model,
            ...
            policy.save(AUXILIARY_POLICY_PATH)
            print('\nPolicy del corrector auxiliar:')
            print(f"  use_auxiliary : {policy.use_auxiliary}")
            print(f"  use_aux_count : {policy.use_aux_count}")
            print(f"  use_aux_duration: {policy.use_aux_duration}")
            print(f"  use_aux_time  : {policy.use_aux_time}")
            print(f"  Guardada en   : {AUXILIARY_POLICY_PATH}")

            aux_full = train_and_save_auxiliary_corrector(
                prepared,
                occurrence_model,
                temporal_model,
                device,
            ...
            print('\nCorrector auxiliar guardado en:')
            print(f"  - {CHECKPOINT_DIR / 'auxiliary_corrector.pt'}")
            if aux_full.metadata:
                print(f"  Ejemplos count/temporal: {aux_full.metadata.get('count_examples', 0)}/{aux_full.metadata.get('temporal_examples', 0)}")
        except Exception as exc:
            disabled = AuxiliaryUsagePolicy(use_auxiliary=False, use_aux_count=False, use_aux_duration=False, use_aux_time=False, metrics={'error': str(exc)})
            disabled.save(AUXILIARY_POLICY_PATH)
            print(f'\nAviso: no se pudo entrenar el corrector auxiliar: {exc}')
            print(f'Policy guardada desactivando el auxiliar en: {AUXILIARY_POLICY_PATH}')
    else:
        disabled = AuxiliaryUsagePolicy(use_auxiliary=False, use_aux_count=False, use_aux_duration=False, use_aux_time=False, metrics={'reason': 'skip_auxiliary_corrector'})
        disabled.save(AUXILIARY_POLICY_PATH)
        print(f'\nCorrector auxiliar omitido. Policy desactivada guardada en: {AUXILIARY_POLICY_PATH}')
