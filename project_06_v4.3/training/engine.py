...
def model_step(model, batch, training: bool = False):
    ...
    return model(
        ...
        anchor_time_bin=batch['anchor_time_bin'],
    )
...
def _format_occurrence_primary_metrics(metrics: dict[str, float]) -> str:
    ordered_keys = (
        'count_exact_acc',
        'count_mae',
        'weekly_total_mae',
    )
    return ' | '.join(f'{k}={metrics[k]:.3f}' for k in ordered_keys if k in metrics)


def _format_occurrence_aux_metrics(metrics: dict[str, float]) -> str:
    ordered_keys = (
        'presence_precision',
        'presence_recall',
        'presence_f1',
...
def fit_model(
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch == 1 or epoch % VERBOSE_EVERY == 0:
            ...
            if model_name == 'OccurrenceModel':
                if train_metrics:
                    print('  Train:', _format_occurrence_primary_metrics(train_metrics))
                if val_metrics:
                    print('  Val  :', _format_occurrence_primary_metrics(val_metrics))

                if 'count_exact_acc' in train_metrics and 'count_exact_acc' in val_metrics:
                    gap = train_metrics['count_exact_acc'] - val_metrics['count_exact_acc']
                    print(f'  train_val_gap_count_exact_acc={gap:.3f}')

                # Opcional: dejar métricas auxiliares en una línea secundaria
                if train_metrics:
                    aux_train = _format_occurrence_aux_metrics(train_metrics)
                    if aux_train:
                        print(f'  Aux Train: {aux_train}')
                if val_metrics:
                    aux_val = _format_occurrence_aux_metrics(val_metrics)
                    if aux_val:
                        print(f'  Aux Val  : {aux_val}')
            else:
                if train_metrics:
