...
def _device_uid(item: dict[str, Any]) -> str | None:
    if 'device_uid' not in item:
        return None
    value = item.get('device_uid')
    if value is None:
        return None
    return str(value)
...
def task_cost(real: dict[str, Any], pred: dict[str, Any]) -> float:
    ...
    if real_name != pred_name or _device_uid(real) != _device_uid(pred):
...
def hungarian_match(true_tasks, pred_tasks):
    ...
    for item in normalized_true:
        true_by_task[(item['device_uid'], item['task_name'])].append(item)
    for item in normalized_pred:
        pred_by_task[(item['device_uid'], item['task_name'])].append(item)

    pairs = []
    for task_key in sorted(set(true_by_task) | set(pred_by_task)):
        tg = sorted(true_by_task.get(task_key, []), key=lambda x: x['start_bin'])
        pg = sorted(pred_by_task.get(task_key, []), key=lambda x: x['start_bin'])
