...
class HybridWeekPredictor:
    def __init__(self, context: GlobalContext, config: dict, occurrence_model, temporal_model, device: torch.device):
        ...
        self.temporal_candidate_source_quotas = {str(k): int(v) for k, v in (config['models']['temporal'].get('candidate_source_quotas', {}) or {}).items() if int(v) > 0}
        self.temporal_prototype_start_offsets = config['models']['temporal'].get('prototype_start_offsets_bins')
        self.temporal_prototype_duration_offsets = config['models']['temporal'].get('prototype_duration_offsets_bins')
        self.temporal_regime_lookback_weeks = int(max(1, config['models']['temporal'].get('regime_lookback_weeks', 8)))
        self.feature_scaling = config.get('feature_scaling', {}) or {}
        self.history_stats = self.feature_scaling.get('history')
        self.occ_numeric_stats = self.feature_scaling.get('occurrence_numeric')
        self.temporal_numeric_stats = self.feature_scaling.get('temporal_numeric')
        self.temporal_candidate_stats = self.feature_scaling.get('temporal_candidate')
        self.use_lag52 = bool(config['models']['occurrence'].get('use_lag52', False))
        self.temporal_architecture = str(config['models']['temporal'].get('architecture', 'ranking')).strip().lower()
        self.temporal_inference_topk_days = max(1, int(config['models']['temporal'].get('inference_topk_days', 4)))
        self.temporal_inference_topk_times = max(1, int(config['models']['temporal'].get('inference_topk_times', 18)))
    ...
    def _predict_counts(self, series: SeriesBundle, template) -> tuple[dict[int, int], dict[str, Any]]:
        ...
        for task_idx, task_name in enumerate(self.context.task_names):
            ...
            numeric, aux = build_occurrence_numeric_features(series, None, task_idx, template_count, support_mean, primary_score, use_lag52=self.use_lag52)
            if self.occ_numeric_stats is not None:
                numeric = transform_feature_matrix(numeric, self.occ_numeric_stats)
    ...
    def _temporal_candidates(self, series: SeriesBundle, planned_slots: list[dict[str, Any]], empirical_template=None) -> list[dict[str, Any]]:
        ...
        for slot in planned_slots:
            ...
            if self.temporal_numeric_stats is not None:
                numeric = transform_feature_matrix(numeric, self.temporal_numeric_stats)
            target_candidate_context = build_target_candidate_context(
                ...
                regime_lookback_weeks=self.temporal_regime_lookback_weeks,
            )
            candidates = gather_empirical_candidates(
                ...
                source_quotas=self.temporal_candidate_source_quotas,
                prototype_start_offsets=self.temporal_prototype_start_offsets,
                prototype_duration_offsets=self.temporal_prototype_duration_offsets,
            )
            candidate_features: list[np.ndarray] = []
            candidate_starts: list[int] = []
            candidate_durations: list[int] = []
            candidate_supports: list[float] = []
            for cand in candidates[:self.temporal_max_candidates]:
                ...
                candidate_supports.append(cand_support)
            if not candidate_features:
                continue
            if self.temporal_candidate_stats is not None:
                candidate_features = [transform_feature_matrix(feat, self.temporal_candidate_stats) for feat in candidate_features]
    ...
    def _temporal_direct_candidates(self, series: SeriesBundle, planned_slots: list[dict[str, Any]]) -> list[dict[str, Any]]:
        if not planned_slots:
            return []
        history = build_future_history_tensor(series, self.window_weeks, bin_minutes=self.bin_minutes)
        if self.history_stats is not None:
        ...
        for slot in planned_slots:
            ...
            rows.append({
                'history': history,
                'task_id': int(slot['task_idx']),
                'database_id': db_idx,
                'robot_id': robot_idx,
                'slot_id': int(slot['slot_id']),
                'anchor_day': int(anchor // self.bins_per_day),
                'anchor_time': int(anchor % self.bins_per_day),
                'numeric_features': numeric,
                'slot': slot,
                'robot_id_value': series.robot_id,
                'task_type_value': self.context.task_names[int(slot['task_idx'])],
            })
        ...
        batch = {
            ...
            'database_id': torch.tensor([r['database_id'] for r in rows], dtype=torch.long, device=self.device),
            'robot_id': torch.tensor([r['robot_id'] for r in rows], dtype=torch.long, device=self.device),
            'slot_id': torch.tensor([r['slot_id'] for r in rows], dtype=torch.long, device=self.device),
            'anchor_day': torch.tensor([r['anchor_day'] for r in rows], dtype=torch.long, device=self.device),
            'anchor_time': torch.tensor([r['anchor_time'] for r in rows], dtype=torch.long, device=self.device),
            'numeric_features': torch.tensor(np.stack([r['numeric_features'] for r in rows]), dtype=torch.float32, device=self.device),
        }
        self.temporal_model.eval()
        with torch.no_grad():
            outputs = self.temporal_model(**batch)
            day_log_probs = torch.log_softmax(outputs['day_logits'], dim=-1).cpu().numpy()
            time_log_probs = torch.log_softmax(outputs['time_logits'], dim=-1).cpu().numpy()
            pred_log_duration = outputs['pred_log_duration'].cpu().numpy()

        scheduled_events = []
        for idx, row in enumerate(rows):
            slot_row = row['slot']
            duration_bins = max(1, int(round(np.expm1(float(pred_log_duration[idx])))))
            day_order = np.argsort(day_log_probs[idx])[::-1][:self.temporal_inference_topk_days]
            time_order = np.argsort(time_log_probs[idx])[::-1][:self.temporal_inference_topk_times]
            candidates = []
            seen_starts: set[int] = set()
            for day_idx in day_order:
                for time_idx in time_order:
                    start_bin = int(day_idx) * self.bins_per_day + int(time_idx)
                    if start_bin in seen_starts:
                        continue
                    seen_starts.add(start_bin)
                    score = float(day_log_probs[idx, day_idx] + time_log_probs[idx, time_idx])
                    candidates.append({
                        'start_bin': start_bin,
                        'duration_bins': duration_bins,
                        'score': score,
                        'model_score': score,
                        'empirical_support': float(slot_row.get('support', 0.0)),
                        'anchor_start_bin': int(slot_row['anchor_start_bin']),
                        'anchor_duration_bins': int(slot_row['anchor_duration_bins']),
                        'slot_id': int(slot_row['slot_id']),
                    })
            candidates.sort(key=lambda x: x['model_score'], reverse=True)
            final_candidates = candidates[:max(1, min(self.temporal_solver_candidates, len(candidates)))]
            if not final_candidates:
                continue
            scheduled_events.append({
                'robot_id': row['robot_id_value'],
                'task_idx': slot_row['task_idx'],
                'task_type': row['task_type_value'],
                'anchor_start_bin': slot_row['anchor_start_bin'],
                'candidates': final_candidates,
            })
        return scheduled_events
    ...
    def predict_series(self, series: SeriesBundle) -> tuple[list[dict], dict]:
        ...
        if self.temporal_architecture in {'direct', 'direct_day_time', 'day_time_direct'}:
            candidate_events = self._temporal_direct_candidates(series, planned_slots)
        else:
        ...
        explanation = {
            ...
            'temporal_decoder': (
                {
                    'mode': 'direct_day_time',
                    'solver_candidates': self.temporal_solver_candidates,
                    'inference_topk_days': self.temporal_inference_topk_days,
                    'inference_topk_times': self.temporal_inference_topk_times,
                }
                if self.temporal_architecture in {'direct', 'direct_day_time', 'day_time_direct'}
                else {
                    ...
                    'solver_candidates': self.temporal_solver_candidates,
                    'candidate_source_quotas': self.temporal_candidate_source_quotas,
                    'prototype_start_offsets_bins': self.temporal_prototype_start_offsets,
                    'prototype_duration_offsets_bins': self.temporal_prototype_duration_offsets,
                    'regime_lookback_weeks': self.temporal_regime_lookback_weeks,
                }
            ),
        }
