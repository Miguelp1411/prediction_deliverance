...
class UnifiedWeekSlotDataset(Dataset):
    ...
    def _build_sample(self, database_id: str, robot_id: str, target_week_idx: int) -> dict[str, Any]:
        ...
        raw_protos = task_slot_prototypes(
            series,
            target_week_idx,
            task_idx,
            max_slots=task_cap,
        ...
        protos = _expand_prototypes(
            raw_protos,
            task_idx=task_idx,
            max_slots=self.max_slots,   # padding global
        ...
        for task_idx in range(self.num_tasks):
            raw_protos = task_slot_prototypes(series, target_week_idx, task_idx, max_slots=self.max_slots, max_weeks=104)
            protos = _expand_prototypes(raw_protos, task_idx=task_idx, max_slots=self.max_slots, bins_per_day=self.bins_per_day)
            prototypes_by_task[task_idx] = protos
            template_count_by_task[task_idx] = int(sum(1 for p in protos if float(p.support) > 0))
            for proto in protos:
                planned_slots.append({
                    ...
                    'pred_task_count': int(template_count_by_task[task_idx]),
        ...
        for task_idx in range(self.num_tasks):
            for proto in prototypes_by_task[task_idx]:
                ...
                numeric = build_temporal_numeric_features(
                    ...
                    pred_task_count=max(1, int(template_count_by_task[task_idx])),
                    template_task_count=int(template_count_by_task[task_idx]),
                ...
                active = 1.0 if evt is not None else 0.0
                if evt is not None:
        ...
        true_counts = (series.counts[target_week_idx].astype(np.float32)
        ...
        return {
            ...
            'query_mask': np.ones(self.query_count, dtype=np.bool_),
            ...
            'target_duration': np.asarray(target_durations, dtype=np.int64),
