...
from proyectos_hibridos.hybrid_schedule_project.src.hybrid_schedule.utils import compose_week_bin
...
@torch.no_grad()
def temporal_metrics(outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], bins_per_day: int, macroblock_bins: int, bin_minutes: int) -> dict[str, float]:
    pred_day = outputs['day_logits'].argmax(dim=-1)
    pred_macro = outputs['macroblock_logits'].argmax(dim=-1)
    pred_fine = outputs['fine_offset_logits'].argmax(dim=-1)
    pred_start = pred_day * bins_per_day + pred_macro * macroblock_bins + pred_fine
...
def evaluate_week(pred_events: list[dict], true_events: list[dict], bin_minutes: int = 5) -> dict[str, float]:
    ...
    matched_true_ids = {id(t) for _, t, _ in matches}
