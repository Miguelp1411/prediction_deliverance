...
def temporal_loss(outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor], duration_loss_weight: float = 0.2) -> torch.Tensor:
    ...
    return day_loss + time_loss + float(duration_loss_weight) * dur_loss
