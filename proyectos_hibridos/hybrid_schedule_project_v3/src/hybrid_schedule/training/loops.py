...
from proyectos_hibridos.hybrid_schedule_project_v3.src.hybrid_schedule.evaluation.metrics import occurrence_metrics, temporal_metrics
from proyectos_hibridos.hybrid_schedule_project_v3.src.hybrid_schedule.training.losses import occurrence_loss, temporal_loss
...
def fit_temporal_model(model, train_loader, val_loader, optimizer, device, epochs, patience, logger, bin_minutes: int, expected_cost_weight: float = 0.30):
    return _fit_loop(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        optimizer=optimizer,
        loss_fn=lambda o, b: temporal_loss(o, b, expected_cost_weight=expected_cost_weight),
