        ...
        for database_id, robot_id, target_week_idx in indices:
            ...
            for task_idx, target_task_events in actual_by_task.items():
                ...
                for slot_rank, target_evt in enumerate(target_task_events):
                    ...
                    total_offset = int(target_evt.start_bin - anchor_start)
                    day_offset = int(np.round(total_offset / self.bins_per_day))
                    day_offset = int(np.clip(day_offset, -self.day_radius, self.day_radius))
                    time_offset = int(total_offset - day_offset * self.bins_per_day)
                    time_offset = int(np.clip(time_offset, -self.time_radius_bins, self.time_radius_bins))
                    numeric = np.asarray([
                    ...
                    self.samples.append({
                        ...
                        'day_target': day_offset + self.day_radius,
                        'time_target': time_offset + self.time_radius_bins,
...
def build_future_occurrence_inputs(
    context: GlobalContext,
    series: SeriesBundle,
    window_weeks: int,
    topk_templates: int,
    ...
    template = build_template_week(series, None, topk=topk_templates)
    ...
    for task_idx in range(len(context.task_names)):
        ...
        support_mean = float(np.mean(support_values)) if support_values else 0.0
        numeric = np.asarray([
            float(template_count), recent['recent_mean'], recent['recent_last'], recent['recent_median'], support_mean, primary_score,
        ], dtype=np.float32)
        samples.append({
            'history': history.astype(np.float32),
            'task_id': task_idx,
