...
def fit_occurrence_model(model, train_loader, val_loader, optimizer, device, epochs, patience, min_delta, logger, max_delta: int):
    return _fit_loop(
        model=model,
...
def fit_temporal_model(model, train_loader, val_loader, optimizer, device, epochs, patience, min_delta, logger, day_radius: int, time_radius_bins: int, bins_per_day: int, bin_minutes: int):
    return _fit_loop(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
