...
class HybridWeekPredictor:
    ...
    def _recent_time_prior(self, series: SeriesBundle, task_idx: int) -> np.ndarray:
        probs = np.ones(self.bins_per_day, dtype=np.float32)
        start = max(0, len(series.week_starts) - 16)
        for week_events in series.events[start:]:
            for evt in week_events:
                if evt.task_idx == task_idx:
                    probs[int(evt.start_bin % self.bins_per_day)] += 1.0
        probs /= probs.sum()
        return probs
