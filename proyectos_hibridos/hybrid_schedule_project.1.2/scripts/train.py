...
from hybrid_schedule.training import (
    OccurrenceDataset,
    TemporalDataset,
    build_database_regularization_profiles,
    build_dataset_sample_weights,
    build_time_split,
    fit_occurrence_model,
    fit_temporal_model,
    render_regularization_markdown,
...
def main() -> None:
    ...
    db_regularization = build_database_regularization_profiles(df, scales=cfg.get('regularization', {}))
    (output_dir / 'database_regularization_profiles.json').write_text(json.dumps(db_regularization, ensure_ascii=False, indent=2), encoding='utf-8')
    (output_dir / 'database_regularization_profiles.md').write_text(render_regularization_markdown(db_regularization), encoding='utf-8')
    ...
    occurrence_ds_train = OccurrenceDataset(
        ...
        db_regularization=db_regularization,
    )
    ...
    if bool(cfg['training'].get('balanced_sampling', True)):
        occ_weights = build_dataset_sample_weights(occurrence_ds_train.samples)
        tmp_weights = build_dataset_sample_weights(temporal_ds_train.samples)
        occ_sampler = WeightedRandomSampler(weights=torch.tensor(occ_weights, dtype=torch.double), num_samples=len(occurrence_ds_train), replacement=True) if occ_weights else None
        tmp_sampler = WeightedRandomSampler(weights=torch.tensor(tmp_weights, dtype=torch.double), num_samples=len(temporal_ds_train), replacement=True) if tmp_weights else None
    ...
    max_grad_norm = float(cfg['training'].get('max_grad_norm', 1.0)) if cfg['training'].get('max_grad_norm', None) is not None else None
    ...
    summary = {
        'profile': profile,
        'regularization_profiles': db_regularization,
        ...
        'artifacts': {
            ...
            'regularization_profile_json': str((output_dir / 'database_regularization_profiles.json').resolve()),
            'regularization_profile_md': str((output_dir / 'database_regularization_profiles.md').resolve()),
        },
    }
