...
def recent_time_distribution(series: SeriesBundle, target_week_idx: int, task_idx: int, bins_per_day: int = 288, max_weeks: int = 16) -> np.ndarray:
    probs = np.ones(int(bins_per_day), dtype=np.float32)
    start = max(0, target_week_idx - max_weeks)
    for week_events in series.events[start:target_week_idx]:
        for evt in week_events:
            if evt.task_idx == task_idx:
                probs[int(evt.start_bin % bins_per_day)] += 1.0
    denom = float(probs.sum())
    if denom <= 0:
        return np.full(int(bins_per_day), 1.0 / max(int(bins_per_day), 1), dtype=np.float32)
    return probs / denom
