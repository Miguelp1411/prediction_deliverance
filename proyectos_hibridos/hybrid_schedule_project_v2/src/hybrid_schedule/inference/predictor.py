...
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.retrieval.template_retriever import build_template_week, propose_extra_slots
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.scheduler import solve_week_schedule
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.utils import index_to_day_offset, index_to_local_offset
...
class HybridWeekPredictor:
    ...
    def _build_planned_slots(self, series: SeriesBundle, template, count_predictions: dict[int, int]) -> list[dict[str, Any]]:
        ...
        for task_idx, pred_count in count_predictions.items():
            ...
            if pred_count > len(chosen):
                extras = propose_extra_slots(series, template, task_idx, None, required=pred_count - len(chosen))
    ...
    def predict_series(self, series: SeriesBundle) -> tuple[list[dict], dict]:
        ...
        explanation = {
            ...
            'temporal_decoder': {
                'mode': 'relative_day_and_local_offsets',
