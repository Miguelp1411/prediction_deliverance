...
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.evaluation.metrics import occurrence_metrics, temporal_metrics
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.training.losses import occurrence_loss, temporal_loss


OCC_EXCLUDE = {'target_count', 'change_target', 'delta_target', 'anchor_start', 'anchor_duration', 'target_start', 'target_duration', 'day_offset_target', 'local_offset_target', 'duration_delta', 'day_offset_radius', 'local_offset_radius', 'template_count'}
TMP_EXCLUDE = {'target_count', 'change_target', 'delta_target', 'anchor_start', 'anchor_duration', 'target_start', 'target_duration', 'day_offset_target', 'local_offset_target', 'duration_delta', 'template_count', 'slot_id', 'baseline_pred_count', 'day_offset_radius', 'local_offset_radius'}
...
def _fit_loop(model, train_loader, val_loader, optimizer, loss_fn: Callable, metrics_fn: Callable, forward_keys_exclude: set[str], device: torch.device, epochs: int, patience: int, model_name: str, logger):
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f'[{model_name}] epoch {epoch:03d} | train_loss={row["train_loss"]:.4f} | val_loss={row["val_loss"]:.4f} | score={selection_score:.3f} | {metrics_str}', flush=True)
...
def fit_temporal_model(model, train_loader, val_loader, optimizer, device, epochs, patience, logger, bins_per_day: int, local_offset_radius: int, bin_minutes: int):
    return _fit_loop(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        ...
        metrics_fn=lambda o, b: temporal_metrics(o, b, bins_per_day=bins_per_day, local_offset_radius=local_offset_radius, bin_minutes=bin_minutes),
