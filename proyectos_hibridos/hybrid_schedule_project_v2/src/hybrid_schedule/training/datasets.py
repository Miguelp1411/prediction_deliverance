...
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.data.features import (
    GlobalContext,
    SeriesBundle,
...
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.retrieval.template_retriever import TemplateWeek, build_template_week, propose_extra_slots
from proyectos_hibridos.hybrid_schedule_project_v2.src.hybrid_schedule.utils import day_offset_to_index, local_offset_to_index
    ...
    def __init__(
        ...
        for database_id, robot_id, target_week_idx in indices:
            ...
            for task_idx, target_task_events in actual_by_task.items():
                ...
                prototypes = template.slot_prototypes_by_task.get(task_idx) or task_slot_prototypes(series, target_week_idx, task_idx)
                if not prototypes:
                    continue
                assignments = assign_events_to_prototypes(target_task_events, prototypes)
                used_slots = {slot_id for slot_id, _event, _proto in assignments}
                proto_map = {proto.slot_id: proto for proto in prototypes}
