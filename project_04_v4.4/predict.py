...
def _merge_candidate_scores(
    ...
    for idx, start_bin in enumerate(anchor_candidates[: max(1, TEMPORAL_NUM_ANCHOR_CANDIDATES)]):
        ...
        distance_penalty = 0.5 * TEMPORAL_ANCHOR_PROXIMITY_WEIGHT * (abs(int(start_bin) - int(anchor_start_bin)) / max(bins_per_day(), 1))
        merged[int(start_bin)] = max(merged.get(int(start_bin), -float('inf')), anchor_score - distance_penalty)
    ...
    for item in ordered_items:
        ...
        for state_score, state_items in beam:
            for candidate_start_bin, candidate_score in candidates:
                ...
                total_score = float(state_score) + float(candidate_score) - float(overlap_penalty) - float(order_penalty)
                next_item = dict(item)
...
def _build_stage_predictions(base_predictions: list[dict], include_repair: bool) -> dict[str, list[dict]]:
    ...
    reranked = _beam_rerank_weekly(_clone_predictions(anchor_predictions))
...
def predict_next_week_staged(occurrence_model, temporal_model, prepared, target_week_idx, device, include_repair: bool | None = None):
    ...
    return _build_stage_predictions(base_predictions, include_repair=include_repair)
...
def predict_next_week(
    ...
    final_stage = 'repair' if ('repair' in stage_outputs and (include_repair is None or include_repair)) else 'rerank'
    if ensemble_stage == 'final':
        selected_stage = final_stage
    else:
        selected_stage = str(ensemble_stage)
