...
def _same_occurrence_start_bin_at_lag(weeks: list[WeekRecord], target_week_index: int, task_id: int, occurrence_index: int, lag_weeks: int) -> int | None:
    source_week = _lag_week_record(weeks, target_week_index, lag_weeks)
    if source_week is None:
        return None
    events = sorted(source_week.events_by_task.get(task_id, []), key=lambda e: (e.start_bin, e.start_time))
    if occurrence_index < len(events):
        return int(events[occurrence_index].start_bin)
    return None


def _same_occurrence_historical_median_bin(history: list[WeekRecord], task_id: int, occurrence_index: int) -> int | None:
    same_occ_bins = _task_occurrence_history_bins(history, task_id, occurrence_index)
    if same_occ_bins:
...
def build_temporal_context(
    ...
    lag26_start = _same_occurrence_start_bin_at_lag(weeks, target_week_index, task_id, occurrence_index, 26)
    lag52_start = _same_occurrence_start_bin_at_lag(weeks, target_week_index, task_id, occurrence_index, 52)
    hist_median_start = _same_occurrence_historical_median_bin(anchor_history, task_id, occurrence_index)
