...
def _build_candidates(anchor_start_bin: int, day_offset_logits_row: torch.Tensor, local_offset_logits_row: torch.Tensor) -> list[tuple[int, float]]:
    day_probs = torch.softmax(day_offset_logits_row, dim=-1)
    local_probs = torch.softmax(local_offset_logits_row, dim=-1)
    day_k = min(PREDICTION_DAY_TOPK, day_probs.numel())
    local_k = min(PREDICTION_TIME_TOPK, local_probs.numel())
    day_values, day_indices = torch.topk(day_probs, k=day_k)
    local_values, local_indices = torch.topk(local_probs, k=local_k)

    anchor_day = int(anchor_start_bin // bins_per_day())
    anchor_local = int(anchor_start_bin % bins_per_day())
    candidates: list[tuple[int, float]] = []
    seen: set[int] = set()
    for d_prob, d_idx in zip(day_values.tolist(), day_indices.tolist()):
        pred_day = max(0, min(6, anchor_day + global_day_index_to_offset(int(d_idx))))
        for l_prob, l_idx in zip(local_values.tolist(), local_indices.tolist()):
            pred_local = max(0, min(bins_per_day() - 1, anchor_local + local_start_index_to_offset(int(l_idx))))
            start_bin = clip_start_bin(pred_day * bins_per_day() + pred_local)
            if start_bin in seen:
                continue
            seen.add(start_bin)
            score = math.log(max(float(d_prob), 1e-9)) + math.log(max(float(l_prob), 1e-9))
            candidates.append((start_bin, score))
    candidates.sort(key=lambda x: x[1], reverse=True)
    return candidates


def build_temporal_inference_plan(occurrence_model, prepared, target_week_idx, device):
    occurrence_model.eval()

    with torch.no_grad():
        seq = _build_sequence(prepared, target_week_idx)
        base_sequence = torch.as_tensor(seq, dtype=torch.float32, device=device).unsqueeze(0)
        count_logits = occurrence_model(base_sequence)
        pred_counts = torch.argmax(count_logits, dim=-1).detach().cpu().numpy()[0]

    task_ids: list[int] = []
    occurrence_slots: list[int] = []
    predicted_count_norms: list[float] = []
    occurrence_progresses: list[float] = []
    history_features: list[np.ndarray] = []
    anchor_days: list[int] = []
    anchor_start_bins: list[int] = []
    ...
    return {
        ...
        'task_ids': task_ids,
        'anchor_start_bins': anchor_start_bins,
        'task_id_tensor': torch.tensor(task_ids, dtype=torch.long, device=device),
        'occurrence_slot_tensor': torch.tensor(occurrence_slots, dtype=torch.long, device=device),
        'history_features_tensor': torch.as_tensor(np.stack(history_features), dtype=torch.float32, device=device),
        'predicted_count_norm_tensor': torch.tensor(predicted_count_norms, dtype=torch.float32, device=device),
        'occurrence_progress_tensor': torch.tensor(occurrence_progresses, dtype=torch.float32, device=device),
        'anchor_day_tensor': torch.tensor(anchor_days, dtype=torch.long, device=device),
    }


def predict_next_week_from_plan(temporal_model, prepared, plan: dict, device):
    temporal_model.eval()
    ...
    with torch.no_grad():
        sequence_context = temporal_model.encode_sequence(plan['base_sequence']).repeat(batch_size, 1)
        outputs = temporal_model.forward_with_context(
            sequence_context=sequence_context,
            task_id=plan['task_id_tensor'],
            occurrence_slot=plan['occurrence_slot_tensor'],
            history_features=plan['history_features_tensor'],
            predicted_count_norm=plan['predicted_count_norm_tensor'],
            occurrence_progress=plan['occurrence_progress_tensor'],
            anchor_day=plan['anchor_day_tensor'],
        )

        predictions = []
        task_ids = plan['task_ids']
        anchor_start_bins = plan['anchor_start_bins']
        for i in range(batch_size):
            ...
            candidates = _build_candidates(
                int(anchor_start_bins[i]),
                outputs['day_offset_logits'][i],
                outputs['local_offset_logits'][i],
            )
            preferred_start_bin = candidates[0][0] if candidates else int(anchor_start_bins[i])
            predictions.append({
                'task_name': task_name,
                'type': task_name,
                'start_bin': int(preferred_start_bin),
...
def predict_next_week(occurrence_model, temporal_model, prepared, target_week_idx, device):
    plan = build_temporal_inference_plan(occurrence_model, prepared, target_week_idx, device)
    return predict_next_week_from_plan(temporal_model, prepared, plan, device)
