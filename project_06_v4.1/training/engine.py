...
def build_adamw(
    ...
    if device.type == 'cuda':
        if use_fused and 'fused' in signature.parameters:
            kwargs['fused'] = True
        elif 'foreach' in signature.parameters:
            kwargs['foreach'] = True
...
def maybe_enable_torch_compile(
    ...
    if not hasattr(torch, 'compile'):
        print(f'[{model_name}] torch.compile no disponible; se usa modo eager.')
        return model
    try:
        compiled = torch.compile(model, mode=mode, dynamic=dynamic)
        print(f'[{model_name}] torch.compile activo (mode={mode}, dynamic={dynamic}).')
        return compiled
    except Exception as exc:  # pragma: no cover - fallback defensivo
        print(f'[{model_name}] torch.compile desactivado por fallback: {exc}')
        return model
