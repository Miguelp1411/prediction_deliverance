...
from config import (
    ...
    TORCH_COMPILE_MODE,
    TRAIN_RATIO,
    TRAIN_COMPUTE_BATCH_METRICS,
    VAL_COMPUTE_BATCH_METRICS,
    OPTIMIZER_USE_FUSED,
    num_time_bins,
    num_day_classes,
...
from predict import build_temporal_inference_plan, predict_next_week, predict_next_week_from_plan
from training.engine import (
    build_adamw,
    evaluate_epoch,
    fit_model,
    maybe_enable_torch_compile,
...
def aggregate_weekly_stats(
    ...
    temporal_model,
    device,
    prediction_plan_cache: dict[int, dict] | None = None,
):
    if not week_indices:
        return {}
    ...
    for idx in week_indices:
        if prediction_plan_cache is not None and int(idx) in prediction_plan_cache:
            pred_week = predict_next_week_from_plan(temporal_model, prepared, prediction_plan_cache[int(idx)], device)
        else:
...
class TemporalE2EEvaluator:
    ...
    def __init__(self, prepared, week_indices, occurrence_model, device):
        ...
        self.device = device
        self.prediction_plan_cache: dict[int, dict] = {}
        total = len(self.week_indices)
        if total > 0:
            print('\nPrecomputando planes E2E de validacion...')
        for pos, idx in enumerate(self.week_indices, start=1):
            self.prediction_plan_cache[int(idx)] = build_temporal_inference_plan(
                self.occurrence_model,
                self.prepared,
                int(idx),
                self.device,
            )
            if pos == total or pos % 10 == 0:
                print(f'  planes listos: {pos}/{total}')
    ...
    def __call__(self, temporal_model):
        ...
        stats = aggregate_weekly_stats(
            ...
            temporal_model,
            self.device,
            prediction_plan_cache=self.prediction_plan_cache,
        )
        compound_score = (
...
def build_occurrence_model(prepared, occ_train: OccurrenceDataset, device: torch.device):
    if OCCURRENCE_MODEL_KIND == 'structured_lag4':
        ...
        target_counts = occ_train.targets if len(occ_train) > 0 else None
...
def main():
    ...
    print(f"torch.compile: {'ON' if ENABLE_TORCH_COMPILE else 'OFF'} (mode={TORCH_COMPILE_MODE})")
    print(f"AdamW fused: {'ON' if OPTIMIZER_USE_FUSED else 'OFF'}")
    ...
    if OCCURRENCE_MODEL_KIND == 'structured_lag4':
        ...
        occurrence_model = maybe_enable_torch_compile(
            occurrence_model,
        ...
        occ_state = fit_model(
            ...
            min_delta=1e-4,
            amp_enabled=False,
            compute_train_metrics=TRAIN_COMPUTE_BATCH_METRICS,
    ...
    temporal_model = maybe_enable_torch_compile(
        temporal_model,
        enabled=ENABLE_TORCH_COMPILE,
        mode=TORCH_COMPILE_MODE,
        dynamic=TORCH_COMPILE_DYNAMIC,
    ...
    temporal_optimizer = build_adamw(
    ...
    tmp_state = fit_model(
        ...
        compute_train_metrics=TRAIN_COMPUTE_BATCH_METRICS,
        compute_val_metrics=VAL_COMPUTE_BATCH_METRICS,
    )
    ...
    occurrence_model_to_save = unwrap_compiled_model(occurrence_model)
    temporal_model_to_save = unwrap_compiled_model(temporal_model)

    save_checkpoint(CHECKPOINT_DIR / 'occurrence_model.pt', 
                    {'state_dict': occurrence_model_to_save.state_dict(), 
                     'metadata': metadata, 
