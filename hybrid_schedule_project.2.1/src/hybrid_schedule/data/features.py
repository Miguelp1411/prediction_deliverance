...
class SeriesBundle:
    ...
    mean_start_p25: np.ndarray  # [W, T]
    mean_start_p75: np.ndarray  # [W, T]
...
def build_global_context(df: pd.DataFrame, bin_minutes: int = 5) -> GlobalContext:
    ...
    for (database_id, robot_id), group in df.groupby(['database_id', 'robot_id']):
        ...
        mean_start_p25 = np.zeros((W, T), dtype=np.float32)
        mean_start_p75 = np.zeros((W, T), dtype=np.float32)
        ...
        for widx in range(W):
            ...
            for tidx in range(T):
                ...
                if task_events:
                    ...
                    mean_start[widx, tidx] = float(np.mean(starts))
                    mean_start_p25[widx, tidx] = float(np.percentile(starts, 25))
                    mean_start_p75[widx, tidx] = float(np.percentile(starts, 75))
        ...
        series[(database_id, robot_id)] = SeriesBundle(
            ...
            mean_start_p25=mean_start_p25,
            mean_start_p75=mean_start_p75,
...
def build_history_tensor(series: SeriesBundle, target_week_idx: int, window_weeks: int) -> np.ndarray:
    ...
    hist_start_p25 = series.mean_start_p25[start:target_week_idx].astype(np.float32) / (7 * 24 * 60 / 5)
    hist_start_p75 = series.mean_start_p75[start:target_week_idx].astype(np.float32) / (7 * 24 * 60 / 5)
    ...
    if not feats:
        return np.zeros((window_weeks, series.counts.shape[1] * 12 + 4), dtype=np.float32)
...
def task_prototype_from_history(series: SeriesBundle, target_week_idx: int, task_idx: int, max_weeks: int = 52) -> dict[str, float]:
    ...
    # Use KDE to find multi-modal density peak instead of basic median
    if len(starts) > 2 and len(set(starts)) > 1:
        from scipy.stats import gaussian_kde
        kde = gaussian_kde(starts, bw_method=0.1)
        grid = np.linspace(min(starts), max(starts), 200)
        best_start = float(grid[np.argmax(kde(grid))])
    else:
        best_start = float(np.median(starts))
