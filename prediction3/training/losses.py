...
from config import (
    ...
    START_CLASS_LOSS_WEIGHT,
    START_LABEL_SMOOTHING,
)
...
class TemporalLoss:
    def __call__(self, outputs: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> torch.Tensor:
        start_loss = F.cross_entropy(
            ...
            batch['target_start_bin'],
            label_smoothing=START_LABEL_SMOOTHING,
        )
        duration_loss = F.smooth_l1_loss(outputs['pred_duration_norm'], batch['target_duration_norm'])
        return START_CLASS_LOSS_WEIGHT * start_loss + DURATION_LOSS_WEIGHT * duration_loss
