...
def _ensure_checkpoint_compatibility(ckpt: dict, prepared, checkpoint_name: str) -> None:
    ...
    if checkpoint_name == 'temporal_model.pt':
        temporal_target_mode = metadata.get('temporal_target_mode')
        has_joint_start_head = 'start_head.weight' in state_dict and 'start_head.bias' in state_dict
        if temporal_target_mode != 'joint_start_bin' or not has_joint_start_head:
            issues.append(
                'el checkpoint temporal usa una salida antigua (day/time separadas) y el código actual espera temporal_target_mode=joint_start_bin con start_head'
            )
...
def _build_candidates(start_logits_row: torch.Tensor) -> list[tuple[int, float]]:
    start_probs = torch.softmax(start_logits_row, dim=-1)
    k = min(PREDICTION_START_TOPK, start_probs.numel())
    values, indices = torch.topk(start_probs, k=k)

    candidates: list[tuple[int, float]] = []
    for prob, idx in zip(values.tolist(), indices.tolist()):
        score = math.log(max(float(prob), 1e-9))
...
def predict_next_week(occurrence_model, temporal_model, prepared, target_week_idx, device, use_repair: bool = False):
    ...
    with torch.no_grad():
        ...
        for i in range(batch_size):
            ...
            candidates = _build_candidates(outputs['start_logits'][i])
